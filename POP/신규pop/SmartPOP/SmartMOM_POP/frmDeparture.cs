...
namespace SmartMOM_POP
{
    public partial class frmDeparture : Form
    {
        ...
        private void btnInput_Click(object sender, EventArgs e)
        {
            if(lblItem.Text == "")
            {
                frmMessage frm1 = new frmMessage("수량입력할 행을 선택해주세요.","AUTOCLOSE");
                frm1.ShowDialog();
                return;
            }
        ...
        private void btnDeparture_Click(object sender, EventArgs e)
        {
            ...
            {
                frmMessage frm = new frmMessage("출발처리 할 대상이 없습니다.", "AUTOCLOSE");
                frm.ShowDialog();
            }
        ...
        private void create_materialDelivery()
        {
            try
            {
                ...
                if (result == null)
                {
                    frmMessage frm = new frmMessage("출발처리 실패하였습니다.", "AUTOCLOSE");
                    frm.ShowDialog();
                    return;
                }
                ...
                {
                    frmMessage frm = new frmMessage("ERROR! : " + result.Rows[0]["p_err_msg"].ToString(), "AUTOCLOSE");
                    frm.ShowDialog();
                    return;
                }

                frmMessage frm1 = new frmMessage("출발처리 완료되었습니다.", "AUTOCLOSE");
                frm1.ShowDialog();
            }
            catch(Exception ex)
            {
                frmMessage frm = new frmMessage(ex.Message.ToString(), "AUTOCLOSE");
                frm.ShowDialog();
            }
        }
