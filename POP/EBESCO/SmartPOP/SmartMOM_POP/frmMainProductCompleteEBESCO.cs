...
{
    public partial class frmMainProductCompleteEBESCO : Form
    {
        string _serialNumber = "";
        string _material_measure_flag = "";
        string _ctQty = "";
        int _gtQty = 0;
        string _lotType = "";
        ...
        public frmMainProductCompleteEBESCO()
        {
            InitializeComponent();
        ...
        private void button_po_search()
        {
            ...
            frmPOSearch frm = new frmPOSearch("ALL"); //작업 지시상태, 작업지시 생성유형
            frm.ShowDialog();
        ...
        private void wo_search(string destinationFlag)
        {
            ...
            paramsMap1.Add("p_err_code", "");                            //p_err_code
            paramsMap1.Add("DIVISION_CD", clsStatic._DIVISION_CD);       //DIVISION_CD
            paramsMap1.Add("COMPANY_CD", clsStatic._COMPANY_CD);         //COMPANY_CD
            paramsMap1.Add("RESOURCE_CD", clsStatic._RESOURCE_CD);      //RESOURCE_CD
            paramsMap1.Add("WORK_ORDER_ID", clsStatic._WORK_ORDER_ID); //WORK_ORDER_ID
            ...
            _material_measure_flag = _woDt.Rows[0]["MATERIALMEASUREFLAG"].ToString();
        ...
        private void btnLabelPrint_Click(object sender, EventArgs e)
        {
            ...
            clsStatic._dialogValue = "";
            frmScale frm = new frmScale(btnPOSearch.Text, lblItem.Text, _sn);
            frm.ShowDialog();
            ...
            // PO 검색 시 측량을 하는 원자재 기준으로 조회 됨으로 인해 아래 소스코드 리마크
            //if (_material_measure_flag == "Y")
            //{
            //    clsStatic._dialogValue = "";
            //    frmScale frm = new frmScale(btnPOSearch.Text, lblItem.Text, _sn);
            //    frm.ShowDialog();
        ...
        private string get_scale_sn()
        {
            ...
            if(retbuf != "NG")
            {
                retbuf = result.Rows[0]["p_err_msg"].ToString();
            }
        ...
        private void button_label_print()
        {
            ...
            double planQty = double.Parse(lblPlanQty.Text.Trim());
            string prodQty = (double.Parse(lblGoodQty.Text.Trim()) + double.Parse(lblMakelotqty.Text)).ToString("#####.##");

            //입력수량(등록할 실적수량)
            string inputQty = lblMakelotqty.Text;

            //실적수량이 계획수량보다 크거나 같을 경우 실적수량 입력 가능한 창 뜨도록 함
            // 무조건 숫자 입력창이 뜨도록 하기 위해 기존 로직 리마크
            ...
            if(clsStatic._dialogValue != "0")
            {
                inputQty = (double.Parse(clsStatic._dialogValue) / 1000).ToString(); ;
                clsStatic._dialogValue = "";
            }
            else
            {
            ...
            try
            {
                ...
                product_insert_new(retbuf, inputQty.Replace(",", ""), "Y");
        ...
        private void product_insert_new(string labelId, string qty, string ganbanyn)
        {
            ...
            paramsMap.Add("p_division_cd", clsStatic._DIVISION_CD);
            paramsMap.Add("p_company_cd", clsStatic._COMPANY_CD);
            paramsMap.Add("p_serialnumber", _sn);
