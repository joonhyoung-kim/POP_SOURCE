...
namespace SmartMOM_POP
{
    public partial class frmScale : Form
    {
        ...
        {
            lblItem.Text = grdMain.Rows[rownum].Cells["품번"].Value.ToString() + " / " + grdMain.Rows[rownum].Cells["품명"].Value.ToString() + " / " + grdMain.Rows[rownum].Cells["소요량"].Value.ToString();
            string scalevalue = grdMain.Rows[rownum].Cells["측정량"].Value.ToString();
            if(scalevalue.Trim() == "")
            {
                scalevalue = "0";
            }
        ...
        {
            //                                   0               1                2       3             4                                   
            string[] headerText = new string[] {  "품번",        "품명",          "단위", "소요량",     "측정량"     }; //5
            ...
            this.Invoke(new MethodInvoker(delegate ()
                {
                    if (clsStatic._WORK_ORDER_ID != "")
                    {
                        ...
                        int rowindex = grdMain.CurrentRow.Index;
                        double buf = double.Parse(grdMain.Rows[rowindex].Cells["측정량"].Value.ToString());
                        grdMain.Rows[rowindex].Cells["측정량"].Value = (buf + double.Parse(readvalue)).ToString();
                        grid_move(rowindex, "DOWN");
        ...
        private void sum_process()
        {
            ...
            {
                string scalevalue = grdMain.Rows[i].Cells["측정량"].Value.ToString();
        ...
        private void btnSelectScaleInit_Click(object sender, EventArgs e)
        {
            ...
            frmMessage frm = new frmMessage($"선태한 품목(" + grdMain.Rows[rowindex].Cells["품번"].Value.ToString() + " / " + grdMain.Rows[rowindex].Cells["품명"].Value.ToString() + " / " + grdMain.Rows[rowindex].Cells["측정량"].Value.ToString() + ") 측량을 초기화 하겠습니까?", "OK_CANCEL");
            DialogResult result = frm.ShowDialog();
            ...
            grdMain.Rows[rowindex].Cells["측정량"].Value = "0";
        }
        ...
        private void btnAllScaleInit_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < grdMain.Rows.Count; i++)
            {
                    grdMain.Rows[i].Cells["측정량"].Value = "0";
            }
        }

        private void lblSum_Click(object sender, EventArgs e)
        {
            grdMain.Rows[0].Cells["측정량"].Value = "1000";
            grdMain.Rows[1].Cells["측정량"].Value = "2000";
            grdMain.Rows[2].Cells["측정량"].Value = "3000";
