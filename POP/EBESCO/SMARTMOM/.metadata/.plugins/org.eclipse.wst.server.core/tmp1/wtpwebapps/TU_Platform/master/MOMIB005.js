...
var MOMIB005 = {
			...
			if(checkedItems.length > 0) {
				micaCommon.messageBox({type:"info", width:"400", height: "145", html:Language.lang['MESSAGES10647'], closeButton:{text:"Close"}, okButton:{text:"OK", 
					after : function(){
						...
						that.cudQualityInfoPop("LD", arrayList);
					}
				}});				
			} else {
	...
	cudQualityInfoPop : function (type, data) {
		var that = this;
		micaCommon.splash.show();
		mom_ajax(type,"reference.itemInfo.qualityInfo.qualityInfo", JSON.stringify(data), that.callback);
	},
	...
	callback :  function(result, data, param) {
