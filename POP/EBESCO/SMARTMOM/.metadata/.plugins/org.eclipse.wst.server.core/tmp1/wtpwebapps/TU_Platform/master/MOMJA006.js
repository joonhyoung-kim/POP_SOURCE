	        ...
	        if(checkedItems.length > 0 && checkFlag != false) {
				micaCommon.messageBox({type:"info", width:"400", height: "145", html:Language.lang['MESSAGES11703'], closeButton:{text:"Close"}, okButton:{text:"OK", 
					after:function(){
						for(var i=0; i<checkedItems.length; i++) {
			        		var param = {
								calOffClassId : checkedItems[i].item.offCd
			        		...
			        		mom_ajax('D', 'reference.workinghours.calOffClass.calOffClass', JSON.stringify(param), that.delCallback, param, callbackChk);
			        	}
