var MOMCC003_1 = {
	...
	init: function() {   
		...
		if(indexInfo != undefined && (indexInfo['op'] == 'inputCancelBtn1' || indexInfo['op'] == 'saveBtn1')) {
    	...
    	// 저장 버튼 클릭 시
    	$(document).on('click', '#rowSaveBtn1', function() {
    		var checkedItems = AUIGrid.getCheckedRowItems(momWidget.grid[0]);
    		var arrayList = [];
    		...
    		if(checkedItems.length > 0) {
    			momWidget.messageBox({type:"info", width:"400", height: "145", html:Language.lang['MESSAGES11194'], closeButton:{text:"Close"}, okButton:{text:"OK", 
					after:function() {
		    			...
		    			mom_ajax('L', 'purchase.materialLedger.materialInputStatus.materialInputStatusSave', JSON.stringify(arrayList), function(result, data) {
    	...
    	mom_ajax('R', 'purchase.materialLedger.materialInputStatus.materialInputStatus', param, function(result, data) {
			if(result != 'SUCCESS' || data.length < 1) {
				return;
			}
