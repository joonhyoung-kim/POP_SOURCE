...
var MOMXX000 = {
	...
	createParam: function(param/*, items, columnFixed*/) {
		...
		var gridProperty = '{'
			...
			+ ',"checkId":"' + $('#checkId').val() + '"'
			+ ',"showRowAllCheckBox":' + ($('#checkId').val() == 'singleRow' || $('#checkId').val() == 'singleRowCell' ? false : true)
	...
	preView: function() {
		...
		$.get(momWidget.contextPath() + queryId, param, function(data) {
			...
			$.get(momWidget.contextPath() + queryId, param, function(data2) {
				...
				var locale = sessionStorage.getItem('locale');
				if(locale != 'KR') {
					for(var i = 0; i < columnLayout1.length; i++) {
						if(columnLayout1[i][locale + 'HeaderText'] != undefined) {
							columnLayout1[i]['headerText'] = columnLayout1[i][locale + 'HeaderText'];
						}
					}
				}
