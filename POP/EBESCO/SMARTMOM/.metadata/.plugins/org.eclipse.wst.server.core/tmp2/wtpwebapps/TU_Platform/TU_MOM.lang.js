...
var Language = {
	...
	init: function(callBack, first) {
		...
		var _divisionCd = sessionStorage.getItem('divisionCd');
		divisionCd = _divisionCd;
		
		var _companyCd = sessionStorage.getItem('companyCd');
		companyCd = _companyCd;
		
		var _locale = sessionStorage.getItem('locale');
		locale = _locale;
		
		if(first) {
			$.get(that.contextPath() + '/mom/request/com.thirautech.mom.lang.language.dummy', {'divisionCd':divisionCd, 'companyCd':companyCd, 'widget':'FRONT', 'locale':locale}, function(data) {
				if(data == undefined) {
					micaCommon.messageBox({type:"danger", width:"400", height: "145", html: "Lang. Config. Not Exist"});
					return;
				}
		...
		if(key.indexOf('+') > 0) {
			...
			if(key.indexOf('@') > 0) {
				...
				if(that.lang[$.trim(tokens[0])] == undefined) {
					return "Multi Language Not Set at " + tokens1[0];
				}
