...
//로그인 한 사용자가 각 부서별 관리자인지 확인(자재/생산/영업)
$.ajax({
	type : 'GET',
	url : tuCommon.contextPath() + '/mom/request/com.thirautech.mom.reference.document.specification.deptCd.dummy',
	timeout : 30000000,
	async : false,
	dataType : 'json',
	contentType : 'application/json; charset=UTF-8',
...
if(deptCd == "Q") { // 품질팀 관리자로 로그인 시에만 QA확인, QA취소 버튼 보이도록
	$("#qaConfirmBtn, #qaCancelBtn").show();
} else {
	$("#qaConfirmBtn, #qaCancelBtn").hide();
}
...
var MOMID001 = {
	init: function() {
		...
		$(document).on("click", "#createBtn", function() {
			...
			$("#updateUserNameModal, #updateDateModal, #salesConfirmDateModal, #productConfirmDateModal, #materialConfirmDateModal, #vendorConfirmDateModal, #confirmDateModal").attr("disabled", true);
			$("#createDateModal").attr("disabled", false);
		});
		...
		$(document).on("click", "#gridModalPopSaveBtn", function() {
			...
			// 자재팀 확인일자 입력 시 접수일자 보다 빠르면 Validation 처리
			if(Date.parse($("#createDateModal").val()) > Date.parse($("#materialConfirmDateModal").val())) {
				micaCommon.messageBox({type:"warning", width:"400", height: "145", html:Language.lang['MESSAGES12424']});
				return;
			}
			
			// 생산팀 확인일자 입력 시 접수일자 보다 빠르면 Validation 처리
			if(Date.parse($("#createDateModal").val()) > Date.parse($("#productConfirmDateModal").val())) {
				micaCommon.messageBox({type:"warning", width:"400", height: "145", html:Language.lang['MESSAGES12425']});
				return;
			}
			
			// 영업팀 확인일자 입력 시 접수일자 보다 빠르면 Validation 처리
			if(Date.parse($("#createDateModal").val()) > Date.parse($("#salesConfirmDateModal").val())) {
				micaCommon.messageBox({type:"warning", width:"400", height: "145", html:Language.lang['MESSAGES12426']});
				return;
			}
			
			// 승인일자 입력 시 접수일자 보다 빠르면 Validation 처리
			if(Date.parse($("#createDateModal").val()) > Date.parse($("#confirmDateModal").val())) {
				micaCommon.messageBox({type:"warning", width:"400", height: "145", html:Language.lang['MESSAGES12423']});
				return;
			}
			
			// 승인일자 입력 시 자재팀 확인일자가 더 늦으면 Validation 처리
			if(Date.parse($("#materialConfirmDateModal").val()) > Date.parse($("#confirmDateModal").val())) {
				micaCommon.messageBox({type:"warning", width:"400", height: "145", html:Language.lang['MESSAGES12420']});
				return;
			}
			
			// 승인일자 입력 시 생산팀 확인일자가 더 늦으면 Validation 처리
			if(Date.parse($("#productConfirmDateModal").val()) > Date.parse($("#confirmDateModal").val())) {
				micaCommon.messageBox({type:"warning", width:"400", height: "145", html:Language.lang['MESSAGES12421']});
				return;
			}
			
			// 승인일자 입력 시 영업팀 확인일자가 더 늦으면 Validation 처리
			if(Date.parse($("#salesConfirmDateModal").val()) > Date.parse($("#confirmDateModal").val())) {
				micaCommon.messageBox({type:"warning", width:"400", height: "145", html:Language.lang['MESSAGES12422']});
				return;
			}
			
			if($("#confirmDateModal").val() != "" && ( $("#materialConfirmDateModal").val() == "" 
													  || $("#productConfirmDateModal").val() == "" 
													  || $("#salesConfirmDateModal").val() == "") ) {
				
				micaCommon.messageBox({type:"warning", width:"400", height: "145", html:Language.lang['MESSAGES12428']});
				return;
			}
		...
		$(document).on("click", ".gridEditBtn", function() {
			...
			$("#specPasswordModal").attr("type", "password");
			$("#updateUserNameModal, #updateDateModal, #salesConfirmDateModal, #productConfirmDateModal, #materialConfirmDateModal, #vendorConfirmDateModal, #confirmDateModal, #createDateModal").attr("disabled", true);
			
			if(deptCd == "M" && $("#confirmDateModal").val() == '') { // 자재팀 관리자로 로그인 시
				$("#materialConfirmDateModal").attr("disabled", false);
			} else if(deptCd == "P" && $("#confirmDateModal").val() == '') { // 생산팀 관리자로 로그인 시
				$("#productConfirmDateModal").attr("disabled", false);
			} else if(deptCd == "S" && $("#confirmDateModal").val() == '') { // 영업팀 관리자로 로그인 시
				$("#salesConfirmDateModal").attr("disabled", false);
			}
			
			// 품질팀 관리자로 로그인하고 자재/생산/영업팀 확인일자에 값이 있을 경우에만 팝업에서 승인일자 수정 가능하도록
			if(deptCd == "Q" && $("#materialConfirmDateModal").val() != null 
					         && $("#productConfirmDateModal").val() != null 
					         && $("#salesConfirmDateModal").val() != null) {
				$("#confirmDateModal").attr("disabled", false);
			}
		...
		$(document).on("click", "#delBtn", function(){
			...
			for(var i = 0; i < checkedItems.length; i++) {
				if(checkedItems[i].item.confirmDate != null) {
					micaCommon.messageBox({type:"warning", width:"400", height: "145", html:Language.lang['MESSAGES12417']});
					return;
				}
			}
		...
		//QA확인 버튼 클릭 시
		$(document).on("click", "#qaConfirmBtn", function() {
			var checkedItems = AUIGrid.getCheckedRowItems("grid");
			var arrayList = [];
			...
			for(var i=0; i<checkedItems.length; i++) {
				if(checkedItems[i].item.qaConfirmFlag == "N") { // QA확인가능여부 N이면 QA확인처리 불가
					micaCommon.messageBox({type:"warning", width:"400", height: "145", html:Language.lang['MESSAGES12414']});
					return;
				}
				
				if(checkedItems[i].item.confirmDate != null) { // 이미 QA확인처리한 경우 재확인처리 불가
					micaCommon.messageBox({type:"warning", width:"400", height: "145", html:Language.lang['MESSAGES12416']});
					return;
				}
				
				if(checkedItems[i].item.materialConfirmDate > get_current_date("YYYY-MM-DD")) { // 자재팀 확인일자가 승인일자보다 늦을 경우 QA확인 불가
					micaCommon.messageBox({type:"warning", width:"400", height: "145", html:"(" + get_current_date("YYYY-MM-DD") + ") " + Language.lang['MESSAGES12420']});
					return;
				}
				
				if(checkedItems[i].item.productConfirmDate > get_current_date("YYYY-MM-DD")) { // 생산팀 확인일자가 승인일자보다 늦을 경우 QA확인 불가
					micaCommon.messageBox({type:"warning", width:"400", height: "145", html:"(" + get_current_date("YYYY-MM-DD") + ") " + Language.lang['MESSAGES12421']});
					return;
				}
				
				if(checkedItems[i].item.salesConfirmDate > get_current_date("YYYY-MM-DD")) { // 영업팀 확인일자가 승인일자보다 늦을 경우 QA확인 불가
					micaCommon.messageBox({type:"warning", width:"400", height: "145", html:"(" + get_current_date("YYYY-MM-DD") + ") " + Language.lang['MESSAGES12422']});
					return;
				}
			}
			
			micaCommon.messageBox({type:"info", width:"400", height: "145", html:Language.lang['MESSAGES12415'], closeButton:{text:"Close"}, okButton:{text:"OK", 
					after:function() {
						for(var i=0; i<checkedItems.length; i++) {
							if(checkedItems[i].item.qaConfirmFlag == "Y") {
								checkedItems[i].item.confirmDate = get_current_date("YYYY-MM-DD");
								arrayList.push(checkedItems[i].item);
							}
						}
						
						mom_ajax("L", "reference.document.specification.specification", JSON.stringify(arrayList), that.callBack, arrayList, "U");
					}
				}
			});
		});
		...
		//QA취소 버튼 클릭 시
		$(document).on("click", "#qaCancelBtn", function() {
			var checkedItems = AUIGrid.getCheckedRowItems("grid");
			var arrayList = [];
			...
			for(var i=0; i<checkedItems.length; i++) {
				if(checkedItems[i].item.confirmDate == null) {
					micaCommon.messageBox({type:"warning", width:"400", height: "145", html:Language.lang['MESSAGES12418']});
					return;
				}
			}
			
			micaCommon.messageBox({type:"info", width:"400", height: "145", html:Language.lang['MESSAGES12419'], closeButton:{text:"Close"}, okButton:{text:"OK", 
					after:function() {
						for(var i=0; i<checkedItems.length; i++) {
							if(checkedItems[i].item.confirmDate != '') {
								checkedItems[i].item.confirmDate = '';
								arrayList.push(checkedItems[i].item);
							}
						}
	...
	comboBox : function(){
		...
		$.get(tuCommon.contextPath() + "/mom/request/com.thirautech.mom.common.comCode.dummy", // 호출 URL
			...
			function(data) {
				options.local = data;
				micaCommon.comboBox.set("#qaConfirm", comboOptions, options);
			}
		);
	},
	...
	getModalSuffixMappingPop : function(data) { 
		var result = {
			...
			confirmDate : $("#confirmDateModal").val(),
