...
var MOMHA005 = {
	...
	grid: function(grid) {
		...
		if(grid == "grid1") {
			...
			AUIGrid.bind(grid, "cellClick", function(e) {
				...
				currencyCd = item[0].item.currencyCd;
	...
	event: function() {
		...
		*/
		...
		//자동생성 버튼
		$("#autoGenerationBtn").click(function() {
			micaCommon.messageBox({type:"info", width:"400", height: "145", html:Language.getLang('MESSAGES12490' + '@' + Language.lang['MESSAGES12489']), closeButton:{text:"Close"}, okButton:{text:"OK", 
				after:function(){
					btnType = 'M';
					var param = {
						inMonth : $("#inMonth").val(),
					...
					mom_ajax("C", "close.materialMonthlyClosing.autoCreateData", JSON.stringify(param), that.callBack, true);
				}
			}});
		});
			...
			if(checkedItems.length > 0) {
				micaCommon.messageBox({type:"info", width:"400", height: "145", html:Language.lang['MESSAGES10649'], closeButton:{text:"Close"}, okButton:{text:"OK", 
					after:function(){
						...
						btnType = 'D';
						mom_ajax("D", "close.materialMonthlyClosing.materialMonthlyClosingDetail", JSON.stringify(param), that.callBack, 'detailDel');
	...
	comboBox: function() {
		...
		$.get(tuCommon.contextPath() + "/mom/request/com.thirautech.mom.common.comCode.dummy", // 호출 URL
				...
				function(data) {
					...
					micaCommon.comboBox.set("#currencyCd",{searchMode:'containsignorecase', autoComplete:true}, options);
				}
		);
	...
	callBack: function(result, data, param, callbackParam) {
...
//					var checkedDetailItems = AUIGrid.getCheckedRowItems("grid2");
			...
			} else if(callbackParam == "detailDel") {
				var checkedDetailItems = AUIGrid.getCheckedRowItems("grid2");
				for(var i = 0; i < checkedDetailItems.length; i++) {
					var param = {
						itemStockInoutId : checkedDetailItems[i].item.itemStockInoutId,
						qty : checkedDetailItems[i].item.qty,
						magamSeq : checkedDetailItems[i].item.magamSeq,
						magamDetailSeq : checkedDetailItems[i].item.magamDetailSeq,
						inoutFlag : locationParam.inoutFlag,
						closeType : closeType
					}
					arrayList.push(param);
				}
				mom_ajax("L", "close.materialMonthlyClosing.materialMonthlyClosingDetail", JSON.stringify(arrayList), that.callBack, "delProc");
			} else if(callbackParam == "delProc") {
				var param = {
						inoutFlag : locationParam.inoutFlag,
						closeType : closeType
				};
				mom_ajax("C", "close.materialMonthlyClosing.materialMonthlyClosingDetailCancel", JSON.stringify(param), that.callBack, "procCall");
			}
		}  else {
