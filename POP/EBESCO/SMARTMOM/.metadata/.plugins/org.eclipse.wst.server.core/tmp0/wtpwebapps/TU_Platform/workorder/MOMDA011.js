...
var MOMDA011 = {
	init: function() {
		...
		$(document).on("click", "#findBtn", function() {
			...
			radioValue = $('input[name="value"]:checked').val();
			var param = mCommon.formGetParam("#form");
			param.isSummary = '';
			if(radioValue == 'summary') {
				param.isSummary = 'Y';
			} 
			mCommon.pageRender("grid", "W201902191110130081000rgQyLvJj5uj", param, function(){}, "workOrder.materialDeductHist.materialDeductHist");
		...
		$(document).on("click", "#excelDownBtn", function() {
			...
			mCommon.getDataset("W201902191110130081000rgQyLvJj5uj/data", param, undefined, function(dataSet) {
				var data = dataSet;
				mCommon.auiGridExcelExport("grid", {fileName: "MATERIAL_DEDUCT_HIST_MOMDA011_" + get_current_date("yyyy-mm-dd"), data: data});
			});
		});
		...
		$(document).on("click", "#deductCancelBtn", function() {
			if(radioValue == 'summary') {
				micaCommon.messageBox({type:"warning", width:"400", height: "145", html: Language.lang['MESSAGES12392']});
				return;
			}
			var checkedItems = AUIGrid.getCheckedRowItems("grid");
			var arrayList = [];
			for(var i = 0; i < checkedItems.length; i++) {
				if(checkedItems[i].item.cancelQty != null) {
