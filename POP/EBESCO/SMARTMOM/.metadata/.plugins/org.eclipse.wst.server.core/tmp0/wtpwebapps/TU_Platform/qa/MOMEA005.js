var MOMEA005 = {
	...
	init: function() {
		...
		momWidget.grid2CustomStyle(data, that.param, indexInfo);
			...
			mom_ajax('R', 'quality.processQualityCoorperation.processQualityCoorperationDown', {workOrderId : that.workOrderId}, function(result, data) {
				...
				if(data.length > 0) {
					for(var i = 0; i < data.length; i++) {
						momWidget.grid2CustomStyle(data, that.param, undefined);
					}
				}
...
$(document).ready(function(event){
	momWidget.init(1, 'MOMEA005', MOMEA005);
	momWidget.init(2, 'MOMEA005', MOMEA005);
	momWidget.init(5, 'MOMEA005', MOMEA005);
	MOMEA005.init();
});