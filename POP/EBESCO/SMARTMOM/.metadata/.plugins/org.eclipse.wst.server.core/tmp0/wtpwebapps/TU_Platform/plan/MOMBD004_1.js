var MOMBD004_1 = {
	...
	init: function() {
		...
		if(this.initParam == undefined || this.initParam['pivot'] == undefined) {
			this.initParam = {pivot : momWidget.getDiff($('#fromDate').val(), $('#toDate').val(), 0, "SUM(DECODE (TO_CHAR(A.PLAN_DATE,'YYYYMMDD')", "A.PLAN_QTY, ''))")};
		}
		...
		if(indexInfo != undefined && indexInfo['op'] == 'findBtn1') {
			...
			$.each(data[0], function(key, value){
				if(key.match('/')) {
					...
					var columnObj = {dataField: key, style: 'right-column', dataType: 'numeric', formatString: '#,###', visible: true, excelHide: true};
...
$(document).ready(function(event){
	momWidget.init(1, 'MOMBD004_1', MOMBD004_1);
	MOMBD004_1.init();
});
