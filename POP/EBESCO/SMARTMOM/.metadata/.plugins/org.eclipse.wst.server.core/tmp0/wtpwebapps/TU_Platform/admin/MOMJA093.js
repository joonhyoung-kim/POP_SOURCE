...
//		});
	...
	event: function() {
		...
		$(document).on("click","#findBtn",function() {
			...
			mCommon.render("grid", "W2018082814222302610009pZV4hZmA66", param, function(){
...
//			cpCd = item.companyCd;
		...
		$(document).on("click", "#gridModalPopSaveBtn", function() {
			...
			mom_ajax('R', 'user.chkUser', {chkUserId : $("#userIdTmpModal").val()}, function(result, data) {
				// 등록 일 때만 Validation 체크(수정은 통과)
				if(that.ajaxType == "post" && data.length > 0) {
					micaCommon.messageBox({type : "warning", width : "400", height : "145", html : Language.lang['MESSAGES12449']});
					return;
				} 
				
				// 수정이거나 등록 시에 이미 등록된 user가 아닐경우엔 실행
				if(that.ajaxType == "put" || data.length == 0) {
					saveData.divisionCd = divisionCd;
					saveData.companyCd = companyCd;
					saveData.userIdTmp = $("#userIdTmpModal").val();
					saveData.description = $("#descriptionModal").val();
	...
	comboBox: function() {
		...
		$.get(tuCommon.contextPath() + "/mom/request/com.thirautech.mom.admin.authority.dummy", // 호출 URL
			function(data) {
				var options = {local: "", textName : "codeName", valueName : "codeId", readonly : true};
				options.local = data;
		...
		//관리자 타입 
		$.get(tuCommon.contextPath() + "/mom/request/com.thirautech.mom.common.comCode.dummy", // 호출 URL
			{codeClassId:"SYSTEM_TYPE"}, // 파라미터
			function(data) {
				var options = {local: "", textName : "name", valueName : "code", readonly : false};
				options.local = data;
				micaCommon.comboBox.set("#userTypeModal", comboOptions, options);
			}
		);
	...
	excelUpload: function() {
		var param = [ { password : mCommon.sha256Set(companyCd)} ];
 		
		excel_upload(file, 'user.user', 'MOMJA093', 'grid', JSON.stringify(param), gvThat.callbackPost, true);
		$("#uploadPop").micaModal("hide");
