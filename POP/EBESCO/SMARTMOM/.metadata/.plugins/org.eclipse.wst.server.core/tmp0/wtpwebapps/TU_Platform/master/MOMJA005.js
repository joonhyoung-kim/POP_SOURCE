...
var MOMJA001 = {
	...
	event: function() {
					...
					after:function(){
						...
						for(var i = 0; i < selectItems.length; i++) {
							...
							param = { 
								...
								shiftType : selectItems[i].item.shiftType
							}
		...
		$(document).on("click", "#dpSaveBtn", function() {
			...
			if(getStartTime > getEndTime) {
				getEndTime = Number(getEndTime) + 2400;
			}
			
			if(offStartTime > offEndTime) {
				offEndTime = Number($("#offEndTime").val()) + 2400;
			}
			...
			if((getStartTime > offStartTime && offStartTime >= getEndTime) || getEndTime < offEndTime) {
	...
	callBackPost : function(result, data, param, callbackParam, flag) {
		...
		if(result == "SUCCESS") {
			if(flag == "groupCall") {
				...
				if(data.useYn == "Y") {
					changeFlag = "Y";
					// Shift코드 사용으로 수정 시 해당 Shift의 비가동유형코드도 사용으로 변경 
				...
				if(changeFlag == "N") {
					...
					var param = {
							...
							endTime : $("#endTime").val(), 
							addDate : checkAddDate
					}
					
					mom_ajax("C", "reference.workinghours.shiftCode.shiftCodeDetails", JSON.stringify(param), this.callBackPostDetail);
				}
			}
			if(flag == "detailCall") {
				...
				$.ajax({
					url : tuCommon.contextPath() + "/mom/request/com.thirautech.mom.reference.workinghours.shiftCode.shiftTime.dummy",
					type : "GET",
					async: false,
					data : {shiftCodeId : $("#shiftCodeIds").val()},
					timeout 	: 30000000,
					dataType 	: 'json',
					contentType : 'application/json; charset=UTF-8',
					success : function(data){
						if(data.length > 0) {
							for(var i=0; i<data.length; i++) {
								startTimes[i] = "";
								startTimes[i] += data[i].startTime;
								endTimes[i] = "";
								endTimes[i] += data[i].endTime;
							}
						}
					},
					error: function(data){},
				...
				for(var i=0; i<startTimes.length; i++) {
					var listParam = {
						...
						endTime : endTimes[i], 
						addDate : checkAddDate[i]
					}
						
					arrayListDetail.push(listParam);
				}
				
				mom_ajax("L", "reference.workinghours.shiftCode.shiftCodeDetails", JSON.stringify(arrayListDetail), this.callBackPostDetail);
			}
			if(flag == true) {
				mCommon.render("grid1", "W201904242350476031000L3cuq9FQbE5", searchParam, function() {
					AUIGrid.setSelectionByIndex("grid1", 0);
	...
	callBackPostDetail : function(){}
};
$(document).ready(function(event){111
	MOMJA001.init();
});