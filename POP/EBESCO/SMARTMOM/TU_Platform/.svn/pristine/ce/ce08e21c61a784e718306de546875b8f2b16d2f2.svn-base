...
//				that.siteList[companyCd] = data[i];
			...
			micaCommon.comboBox.set("#field-3", {width : 165, dropDownHeight : '250px', autoDropDownHeight : false, searchMode : 'containsignorecase', autoComplete : true, selectedIndex : 0}, siteOptions);
			...
			$("#field-3").on('change', function (event) {
				...
				$.get(tuCommon.contextPath() + '/mom/request/com.thirautech.mom.common.comCode.dummy', param, function(data) {
					...
					micaCommon.comboBox.set("#field-4", {width: 165}, langOptions);
					$("#field-4").jqxComboBox('selectIndex', 0);
				});
			});
			
			var site = Login.siteList[$("#field-3").val()];
			var param = {divisionCd : site.divisionCd, companyCd : site.companyCd, codeClassId : "LANGUAGE_CODE"};
			...
			success: function(data){
				if(data == "Success") {
					...
					$.get("/TU_Platform/system/loginInfo", function(data) {
						...
						sessionStorage.setItem("menuList", JSON.stringify(data.menuList)); // 사이트메뉴 리스트
