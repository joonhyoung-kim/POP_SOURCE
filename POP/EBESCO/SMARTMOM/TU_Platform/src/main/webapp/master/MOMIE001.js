...
var MOMIE001 = {
	...
	grid: function() {
		...
		$(document).on("click","#findBtn",function(){
			/* Begin.., Mica Param으로 강제 박혀 있는 건데, Tibero 에서 안되어져 있어서.., 그리고 XMOM 사용시 불필요 */
 			var param = mCommon.formGetParam("#form");
			param['endDate'] = '2018-01-01';
			/* End.., Mica Param으로 강제 박혀 있는 건데, Tibero 에서 안되어져 있어서.., 그리고 XMOM 사용시 불필요 */
			mCommon.render('grid', 'W201807031646441791003Fy7hsfb8Avo',  param, function(){});
		});
		...
		$(document).on("click", "#gridModalPopSaveBtn", function() {
			...
			if(!that.timeCheck($("#startTimeModal").val())){
				micaCommon.messageBox({type:"warning", width:"400", height: "145", html:Language.lang['MESSAGES10786'] + " -> " + Language.lang['MESSAGES10038']});
				return;
			}
			
			if(!that.timeCheck($("#endTimeModal").val())){
				micaCommon.messageBox({type:"warning", width:"400", height: "145", html:Language.lang['MESSAGES10922'] + " -> " + Language.lang['MESSAGES10038']});
				return;
			}
			...
			if(checkedItems.length > 0) {
				micaCommon.messageBox({type:"info", width:"400", height: "145", html:Language.lang['MESSAGES11743'], closeButton:{text:"Close"}, okButton:{text:"OK", 
					after:function(){
						...
						mom_ajax("L", "reference.workinghours.shift.shift", JSON.stringify(arrayList), that.callback);
	...
	comboBox: function() {
		...
		mCommon.comboBoxClickCall("#resourceName, #resourceCdModal", function(callBack) {
			$.get(tuCommon.contextPath() + "/mom/request/com.thirautech.mom.reference.workinghours.shiftPlanSchedule.shiftResource.dummy", // 호출 URL
				{}, // 파라미터
				function(data) {
					options.local = data;
	...
	setShiftPop: function(data) { // data가 비었으면 초기화.
		...
		// 시작시간을 설정해준다.
		$("#startTimeModal").val(startTime || "");
		// 완료시간을 설정해준다.
		$("#endTimeModal").val(endTime || "");
		// 적용시작일자를 설정해준다.
