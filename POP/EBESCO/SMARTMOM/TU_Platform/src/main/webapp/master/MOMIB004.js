...
var MOMIB004 = {
	...
	grid: function() {
		...
		AUIGrid.bind("grid", "rowAllChkClick", function( event ) {
			if(indeterminate) {
				AUIGrid.setCheckedRowsByValue(event.pid, "productClassId", []);
				indeterminate = false;
			} else {
				var item = AUIGrid.getGridData("grid");
				var currFirRow = (currPage - 1) * 100; 
				var currlastRow = currPage * 100; 
				
				for(var i = currFirRow; i < currlastRow; i++) {
					if(item.length > i) {
						var num = item[i].num;
						AUIGrid.addCheckedRowsByValue(event.pid, "num", num);
					}
				}
				indeterminate = true;
			}
		 ...
		 AUIGrid.bind("grid", "pageChange", function(event) {
			 currPage = event.currentPage;
			 if(currPage == 0) {
				 currPage = 1;
			 }
		 });
	...
	event: function() {
		...
		$(document).on("click", "#createBtn", function() {
			...
			$("#itemIdModal, #customerCdModal, #destinationCdModal").jqxComboBox({disabled: false});
		...
		$(document).on("click", "#gridModalPopSaveBtn", function() {
			...
			micaCommon.messageBox({type:"info", width:"400", height: "145", html:Language.lang['MESSAGES11194'], closeButton:{text:"Close"}, okButton:{text:"OK", 
				after:function(){
					...
					that.cudModalSuffixMappingPop(type, param);
				}
			}});
			...
			if(checkedItems.length > 0) {
				micaCommon.messageBox({type:"info", width:"400", height: "145", html:Language.lang['MESSAGES10648'], closeButton:{text:"Close"}, okButton:{text:"OK", 
					after:function(){
						...
						that.cudModalSuffixMappingPop("LD", arrayList);
					}
				}});				
			} else {
		...
		$(document).on("click", ".gridEditBtn", function() {
			...
			$("#itemIdModal, #customerCdModal, #destinationCdModal").jqxComboBox({disabled: true});
	...
	cudModalSuffixMappingPop : function (type, data) {
		micaCommon.splash.show();
		// modelSuffixMapping정보 CUD 해주는 곳이다.
		// process 호출하여 post를 넘겨주면 데이터가 생성.
		mom_ajax(type,"reference.itemInfo.modelSuffixMapping.modelSuffixMapping", JSON.stringify(data), this.callbackPost);
	},
