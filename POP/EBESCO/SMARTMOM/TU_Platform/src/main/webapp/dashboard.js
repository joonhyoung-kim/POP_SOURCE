...
//					{value : dUomNum}, 
		...
		$.get(tuCommon.contextPath() + "/mom/request/com.thirautech.mom.common.comCode.dummy", 
			{codeClassId:"DASHBOARD_SETUP", codeId:"D_UOM_STOCK"}, 
			function (data) {
				data = data[0];
				dUomStock = data.name;
				dUomNumStock = data.value;
				...
				var chartTopOption = {"tooltip":{"trigger":"axis","axisPointer":{"type":"cross","crossStyle":{"color":"#999"}}},"toolbox":{"feature":{"dataView":{"show":true,"readOnly":false},"magicType":{"show":true,"type":["line","bar"]},"restore":{"show":true},"saveAsImage":{"show":true}}},
						...
						"yAxis":[{"type":"value","name":Language.lang[dUomStock],"axisLabel":{"formatter":"{value}","show":true},"splitLine":{"show":true},"axisLine":{"show":true},"axisTick":{"show":true,"alignWithLabel":false},"silent":false,"boundaryGap":false},
				...
				//상단 차트
				$.get(tuCommon.contextPath() + "/mom/request/com.thirautech.mom.addition.dashboard.stockStatus.dummy", 
					{value : dUomNumStock}, 
					function (data) {
						for(var i = 0; i < data.length; i++) {
							var dataI = data[i];
