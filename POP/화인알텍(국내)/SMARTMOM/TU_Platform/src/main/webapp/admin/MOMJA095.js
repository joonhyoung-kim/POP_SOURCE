...
//							  chkFlag = true;
//						  }
						  arrayList.push(editedRowItems[i]);
					  }
					  mom_ajax("LU", "siteMenu.siteMenu", JSON.stringify(arrayList), that.siteSaveCallback);
					}
			}});
		});
...
//							  }
					  ...
					  if(cntDel > 0) {
						  ...
						  mom_ajax("LD", "siteMenu.siteMenu", JSON.stringify(removeArray), that.delCallback, editedRowItems[i], flag);
					  }
					  if(createArray.length > 0) {
						  mom_ajax("L", "siteMenu.siteMenu", JSON.stringify(createArray), that.siteSaveCallback, true);
					  }
					}
			  }});
//			  
...
//			  }
		...
		$(document).on("click", "#excelDownBtn", function() {			
			mCommon.auiGridExcelExport("grid2", {fileName : "SITEMENU_MOMJA095_" + get_current_date("yyyy-mm-dd")});
		});
		
	},
	siteSaveCallback: function(result, data, param, callBackParam) {
...
//			}
	...
	delCallback: function(result, data, params, callBackParam, flag) {
		var that = this.MOMJA095;
		if(result == 'SUCCESS'){
			var param = {
					siteMenuDivisionCd : siteMenuDivisionCd, 
