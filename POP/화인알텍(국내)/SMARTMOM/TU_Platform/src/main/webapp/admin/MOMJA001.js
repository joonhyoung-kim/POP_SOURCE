...
var MOMJA001 = {
	...
	event: function() {
		...
		$(document).on("click", ".grid1EditBtn", function() {
			...
			if(item.isSystem == "Y" && empAuthority != "99999999" ){
				micaCommon.messageBox({type:"warning", width:"400", height: "145", html:Language.lang['MESSAGES11884']});
				return false;
			}
		...
		$(document).on("click", "#dCreateBtn", function() {
			...
			if(selectItems[0].item.isSystem == "Y" && empAuthority != "99999999" ){
				micaCommon.messageBox({type:"warning", width:"400", height: "145", html:Language.lang['MESSAGES11885']});
				return false;
			}
		...
		$(document).on("click", "#dCopyBtn", function() {
			...
			var selectGroupItems = AUIGrid.getSelectedItems("grid1");
			// 선택된 값이 0개라면 이벤트 실행취소.
			if(selectItems.length < 1 || selectItems.length > 1) {
				var message = "";
				if(selectItems.length > 1) message = Language.lang['MESSAGES11603'];  
			...
			if(selectGroupItems[0].item.isSystem == "Y" && empAuthority != "99999999" ){
				micaCommon.messageBox({type:"warning", width:"400", height: "145", html:Language.lang['MESSAGES11885']});
				return false;
			}
		...
		$(document).on("click", "#dExcelDownBtn", function() {
			mCommon.auiGridExcelExport("grid2", {fileName : "CODE_DETAIL_MOMJA001_" + get_current_date("yyyy-mm-dd")});
		});
	...
	setCodeDetailPop: function(data) { // data가 비었으면 초기화.
		...
		$("#attribute21").val(data.attribute21 || "");
		$("#attribute22").val(data.attribute22 || "");
		$("#attribute23").val(data.attribute23 || "");
		$("#attribute24").val(data.attribute24 || "");
		$("#attribute25").val(data.attribute25 || "");
		$("#attribute26").val(data.attribute26 || "");
		$("#attribute27").val(data.attribute27 || "");
		$("#attribute28").val(data.attribute28 || "");
		$("#attribute29").val(data.attribute29 || "");
		$("#attribute30").val(data.attribute30 || "");
		// 비고 값을 설정해준다.
		$("#dpDescription").val(data.description || "");
		
		// 코드그룹, 수정자, 수정시간은 사용자가 수정을 못하기에 읽기전용으로 선언해준다.
		$("#codeClassIds").attr("readonly","readonly");
		...
		var result ={
			...
			attribute21 : $("#attribute21").val(),
			attribute22 : $("#attribute22").val(),
			attribute23 : $("#attribute23").val(),
			attribute24 : $("#attribute24").val(),
			attribute25 : $("#attribute25").val(),
			attribute26 : $("#attribute26").val(),
			attribute27 : $("#attribute27").val(),
			attribute28 : $("#attribute28").val(),
			attribute29 : $("#attribute29").val(),
			attribute30 : $("#attribute30").val(),
			updateBy : userId,
