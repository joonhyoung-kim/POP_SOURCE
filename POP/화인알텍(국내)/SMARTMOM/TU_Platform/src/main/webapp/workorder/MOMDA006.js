...
var MOMDA006 = {
	...
	design : function (){
		...
		} else if (locationParam.itemType == "EM") {
			$("#label-id").text(Language.lang['MESSAGES12254']);
		} else {
	...
	grid: function() {
		...
		//I/F
		$(document).on("click", "#ifBtn", function() {
			micaCommon.messageBox({type:"info", width:"400", height: "145", html:Language.lang['MESSAGES12100'], closeButton:{text:"Close"}, okButton:{text:"OK", 
				after:function(){
					var param = {};
					mom_ajax("C", "purchase.stock.itemStockStatus.stockIf", JSON.stringify(param), that.callbackPost);
				}
			}});
		});
	...
	comboBox: function(){
		...
		var facComboOption;
		var itemComboOption;
		if(locationParam.itemType == "EM") {
			paramItemType = {codeClassId:"ITEM_TYPE", attribute12:"Y"}
			paramItem = {divisionCd: divisionCd, itemType: "'EM'"};
			facComboOption = {searchMode:'containsignorecase', autoComplete:true, checkboxes: true, checkedIndex:[0,1]};
			itemComboOption = {selectedIndex:0, searchMode:'containsignorecase', autoComplete:true};
		} else {
			paramItemType = {codeClassId:"ITEM_TYPE", codeId: "'RM','SM','CI','GI','FP','SP'"};
			paramItem = {divisionCd: divisionCd};
			facComboOption = {searchMode:'containsignorecase', autoComplete:true, checkboxes: true, checkedIndex:[0,1,2,4]};
			itemComboOption = {searchMode:'containsignorecase', autoComplete:true};
		}
		// 창고
		mCommon.comboBoxClickCall("#locationName, #moLocation", function(callBack) {
			...
			$.get(tuCommon.contextPath() + "/mom/request/com.thirautech.mom.common.specifyCode.dummy", // 호출 URL
				...
				function(data) {
					options.local = data;
					micaCommon.comboBox.set("#type",itemComboOption, options);
...
//		);
			...
			optItems.param = paramItem; // 기본 파라미터
		...
		$.get(tuCommon.contextPath() + "/mom/request/com.thirautech.mom.common.comCode.dummy", // 호출 URL
			...
			function(data) {
				options.local = data;
				micaCommon.comboBox.set("#facilityType",facComboOption, options);
			}
		);
	...
	getSearchData : function(){
		...
		var param = {
			...
			itemType : locationParam.itemType
		}
		return param;
	},
	callbackPost : function(result, data){
		var that = this.MOMDA006;
		if(result == "SUCCESS"){
