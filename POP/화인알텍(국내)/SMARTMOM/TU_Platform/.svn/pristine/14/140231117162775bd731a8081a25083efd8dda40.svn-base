...
//			},
		...
		if(indexInfo != undefined && indexInfo['op'] == 'findBtn1') {
			...
			this.initParam = {weekStartDate : $("#week").val().replace(/-/gi, '')
							, pivot1 : momWidget.getDiff2($("#week").val(), 1, "DECODE(MOD(ROWNUM,3),0,M#||'%',M#)")
							, pivot2 : momWidget.getDiff2($("#week").val(), 2, "DECODE(MOD(ROWNUM,3),0,W#||'%',W#)")
							, pivot3 : momWidget.getDiff2($("#week").val(), 3, "DECODE(MOD(ROWNUM,3),0,D#||'%',D#)")
							};
