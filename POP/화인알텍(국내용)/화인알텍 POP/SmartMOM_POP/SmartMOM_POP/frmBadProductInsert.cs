...
namespace SmartMOM_POP
{
    public partial class frmBadProductInsert : Form
    {
        ...
        public frmBadProductInsert(string po, string badlotqty, DataTable wobadDt)
        {
            InitializeComponent();
        ...
        private void txtSN_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                if (_flag == "BADPRODUCT")
                {
                    DataRow[] drs1 = _wobadDt.Select("NONTYPE='" + txtSN.Text.Trim() + "'");
                    if (drs1.Length > 0)
                    {
                    ...
                    {
                        frmMessage frm = new frmMessage("정확한 불량코드를 입력하여 주세요.", "AUTOCLOSE");
                        frm.ShowDialog();
                        txtSN.Focus();
                        txtSN.SelectAll();
                        return;
        ...
        private void bad_insert(string sn)
        {
