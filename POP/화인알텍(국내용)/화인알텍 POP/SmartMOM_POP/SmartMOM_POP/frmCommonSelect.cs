    ...
    public partial class frmCommonSelect : Form
    {
        ...
        public frmCommonSelect(string selectType)
        {
            ...
            if (clsStatic._resouceType == "LINE")
            {
                ...
                {
                    _buttonDt.Dispose();
                    _buttonDt = new DataTable();
                    _buttonDt.Columns.Add("code");
                    _buttonDt.Columns.Add("name");
                    _buttonDt.Columns.Add("value");
                    _buttonDt.Columns.Add("codeName");
                    _buttonDt.Columns.Add("buttontext");

                    _buttonDt.Rows.Add("자재입고", "자재입고", "자재입고", "자재입고", "자재입고");
                    _buttonDt.Rows.Add("간판라벨발행", "간판라벨발행", "간판라벨발행", "간판라벨발행", "간판라벨발행");
                    _buttonDt.Rows.Add("원자재불출", "원자재불출", "원자재불출", "원자재불출", "원자재불출");
                    _buttonDt.Rows.Add("재고이동/공정이동", "재고이동/공정이동", "재고이동/공정이동", "재고이동/공정이동", "재고이동/공정이동");
                    _buttonDt.Rows.Add("Pallet구성", "Pallet구성", "Pallet구성", "Pallet구성", "Pallet구성");
                    _buttonDt.Rows.Add("출하처리", "출하처리", "출하처리", "출하처리", "출하처리");
                    _buttonDt.Rows.Add("생산이력조회", "생산이력조회", "생산이력조회", "생산이력조회", "생산이력조회");
                    button_init();

                    //btnPrev.Visible = false;
                    //btnNext.Visible = false;
                    //btnBody01.Text = "자재입고";
                    //btnBody02.Text = "간판라벨발행";
                    //btnBody03.Text = "원자재불출";
                    //btnBody04.Text = "재고이동/공정이동";
                    //btnBody05.Text = "Pallet구성";
                    //btnBody06.Text = "출하처리";
                    //btnBody07.Text = "공정품불출";
                    //btnBody07.Visible = true;
        ...
        private void stock_process(string stockType)
        {
            if (stockType == "FAC400")
            {
                _buttonDt = getStockListInfo(stockType);
            }
            else
            {
        ...
        private DataTable getStockListInfo(string facilitytype)
        {
            ...
            paramsMap.Add("FACILITY_TYPE", facilitytype);
            ...
            dt = clsUtil.GetServiceData("com.thirautech.mom.pop.popResult.getstocklist.dummy", paramsList, clsStatic._serviceSelectURL, ref retvalue);
        ...
        {
            _buttonDt = clsStatic.getComCodeListInfo("ITEM_TYPE", "");
        ...
        private void btnBody_Click(object sender, EventArgs e)
        {
            ...
            else if (clsStatic._resouceType == "TO" || clsStatic._resouceType == "PRODUCT_INPUT")
            {
                string[] strs = buf.Split('(');
