...
namespace SmartMOM_POP
{
    public partial class ucMoveProductItemAction : UserControl
    {
        ...
        private void search_process(string item_id, string qtybuf)
        {
            ...
            paramsMap.Add("ITEM_ID",     _scanitem_id);
            paramsMap.Add("GANBAN_ID",   _scanganban);
            paramsMap.Add("PALLETID",    _scanpalletid);
            paramsMap.Add("CT",          _scanct);
            paramsMap.Add("SCANQTY",     "0");
        ...
        private void scan_search(string sn, string ct, string palletid, string labeltype)
        {
            ...
            else
            {
                ...
                dt = clsUtil.GetServiceData("com.thirautech.mom.pop.popResult.get_itemStockMove_snctptQTY1.dummy", paramsList, clsStatic._serviceSelectURL, ref retvalue);
            }
            ...
            if(dt.Rows.Count > 0)
            {
                ...
                {
                    clsStatic._moveitemDt = clsStatic.getComCodeListInfo("ITEM_TYPE", "");
                }
                ...
                DataRow[] drs = clsStatic._moveitemDt.Select("CODE = '" + dt.Rows[0]["ITEMTYPE"].ToString()+ "'");
                if(drs.Length == 0)
                {
                    string message = clsStatic._STOCK_TYPE_NAME + "에서는 처리가 불가능한 품목입니다.(가능품목:";
            ...
            if (checkbuf == "NO")
            {
                try
                {
                    if (dt.Rows[0]["wospec"].ToString() != "")
                    {
                        frmMessage frm = new frmMessage(dt.Rows[0]["WOSPEC"].ToString().Trim() + "이 필요한 제품입니다.", "AUTOCLOSE");
                        frm.ShowDialog();
                    }
                }
                catch
                ...
                try
                {
                    lblProductOrder.Text = dt.Rows[0]["productorderid"].ToString();
                }
                catch
                {
        ...
        private void moveProduct_main()
        {
            ...
            else if(txtSN.Text.Trim().Length > 8)
            {
                labeltype = "CT";
                _scansn = "";
                _scanct = txtSN.Text.Trim().Substring(txtSN.Text.Trim().Length - 8, 8);
                txtSN.Text = _scanct;
                _sn = _scanct;
                _scanpalletid = "";
                _scanitem_id = "";
            ...
            else
            {
                ...
                _scanct = "";
                _scanpalletid = "";
                _scanitem_id = txtSN.Text.Trim();
            }
            if (labeltype != "ITEM")
            {
                scan_search(_scansn, _scanct, _scanpalletid, labeltype);
        ...
        private string get_moveSeq()
        {
            ...
            DataTable checkDt = clsUtil.GetServiceData("com.thirautech.mom.pop.popResult.getMoveseqCheck1.dummy", paramsCheckList, clsStatic._serviceUsertURL, ref retvalue);
            ...
            if(int.Parse(checkDt.Rows[0]["p_runCount"].ToString().Trim()) > 0)
            {
                return "NG";
            }
        ...
        private void move_transfer(string move_id)
        {
            ...
            paramsList.Add(paramsMap);
            string retvalue = "";
            DataTable moveDt = clsUtil.GetServiceData("com.thirautech.mom.pop.popResult.create_move_proc1.dummy", paramsList, clsStatic._serviceUsertURL, ref retvalue);
        ...
        private void btnTransfer_Click(object sender, EventArgs e)
        {
            ...
            if(retbuf == "NG")
            {
                frmMessage frm1 = new frmMessage("현재 이동처리가 진행중입니다. 잠시후(1분 후) 이동처리 진행 하여 주세요.", "AUTOCLOSE");
                frm1.ShowDialog();
                txtSN_Focus();
                return;
            }
