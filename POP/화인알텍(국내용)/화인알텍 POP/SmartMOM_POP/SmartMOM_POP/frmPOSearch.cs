...
namespace SmartMOM_POP
{
    public partial class frmPOSearch : Form
    {
        ...
        private void InitLabelList()
        {
            ...
            string[] headerText = new string[] {  "고객사제번",     "ECO",    "품목",    "품명",     "규격",          "지시량",     "잔량수량", "작업지시번호", "작업일자",        "양품수량",   "불량수량",    "작업지시유형", "상태",         "입력기본수량",  "입력방식",     "CT구성수량",   "GT라벨ID",      "CT라벨ID",      "PALLET라벨ID",     "향지"           }; //19
            string[] columnName = new string[] {  "PRODUCTORDERID", "WOSPEC", "ITEMID",  "ITEMNAME", "SPECIFICATION", "CONFIRMQTY", "REMAINQTY","WORKORDERID",  "PLANDATE",        "QTY",        "BADQTY",      "WOTYPENAME",   "WOSTATENAME",  "POPMAKELOTQTY", "POPINPUTTYPE", "POPCTQTY",     "POPGTLABELID",  "POPCTLABELID",  "POPPALLETLABELID", "POPDESTINATION" };
            string[] columnType = new string[] {  "T",              "T",      "T",       "T",        "T",             "T",          "T",        "T",            "T",               "T",          "T",           "T",            "T",            "T",             "T",            "T",            "T",             "T",             "T",                "T"              };
                                                                  
            int[] columnWidth    = new int[]   {  248,              215,      248,       450,        400,             130,          130,        290,            210,               80,           80,            100,            230,            100,             100,            100,            100,             100,             100,                100              };
            ...
            string[] cellAlign = new string[]  {  "C",              "L",      "L",       "L",        "L",             "R",          "R",        "C",            "C",               "R",          "R",           "C",            "C",            "C",             "C",            "C",            "C",             "C",             "C",                "C"              };
        ...
        private void grdMain_Color_change()
        {
            ...
            {
                listdate = grdMain["작업일자", i].Value.ToString().Replace(",", "");

                if (listdate != strDate)
                {
                    for (int j = 0; j < grdMain.ColumnCount; j++)
                    {
                        grdMain[j, i].Style.BackColor = Color.LightGray;
                    }
                }
            }
        }
        ...
        private void wo_search()
        {
            ...
            DataTable woDt = clsUtil.GetServiceData("com.thirautech.mom.pop.popResult.get_popwo_list1.dummy", paramsList, clsStatic._serviceSelectURL, ref retvalue);
        ...
        private void btnOK_Click(object sender, EventArgs e)
        {
            if (_wo != "")
            {
                ...
                string destinationbuf = grdMain.Rows[rowIndex].Cells["상태"].Value.ToString();

                if(destinationbuf == "작업중단")
                {
                    clsStatic._DESTINATION = grdMain.Rows[rowIndex].Cells["향지"].Value.ToString();
                }
                ...
                clsStatic._WORK_ORDER_ID = _wo;
                clsStatic._dialogValue = grdMain.Rows[rowIndex].Cells["ECO"].Value.ToString();
