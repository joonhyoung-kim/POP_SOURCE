...
namespace SmartMOM_POP
{
    public partial class frmMainProductComplete : Form
    {
        ...
        private void wo_search(string destinationFlag)
        {
            ...
            _gtDt.Dispose();
            _gtDt = new DataTable();
            _gtDt = clsUtil.GetServiceData("com.thirautech.mom.pop.popResult.get_popgt_list.dummy", paramsList2, clsStatic._serviceSelectURL, ref retvalue);
        ...
        {
            if (_gtDt.Rows.Count > 0)
            {
                string ct = _gtDt.Rows[0]["박스포장번호"].ToString();
