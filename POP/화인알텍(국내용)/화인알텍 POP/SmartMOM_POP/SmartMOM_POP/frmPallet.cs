...
namespace SmartMOM_POP
{
    public partial class frmPallet : Form
    {
        ...
        private void txtSN_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                else
                {
                    ...
                    {
                        frmMessage frm1 = new frmMessage("이미 입력하신 팔레트/박스번호 입니다.", "AUTOCLOSE");
                        frm1.ShowDialog();
                        return;
                    }
        ...
        private void product_insert(string sn, string labeltype, string palletid)
        {
            ...
            DataTable dt = clsUtil.GetServiceData("com.thirautech.mom.pop.popResult.create_palletupsert_proc1.dummy", paramsList, clsStatic._serviceUsertURL, ref retvalue);
        ...
        private void btnLabelPrint_Click(object sender, EventArgs e)
        {
            if (grdPallet.Rows.Count > 0)
            {
                ...
                clsLabelSet.label_print(clsStatic._MACADDRESS, clsStatic._PRINTID, _palletid, "", "", "", "", ref _BasicDs);

                frmMessage frm1 = new frmMessage("OK버튼을 누르면 제품 재고이동 처리를 진행합니다.", "OK");
                DialogResult result = frm1.ShowDialog();

                frmMoveProductProcess frm2 = new frmMoveProductProcess(clsStatic._FROMLINE, clsStatic._TOLINE, _itemtype, clsStatic._ITEMTYPE, lblPallet.Text, lblProductOrder.Text);
                frm2.ShowDialog();
            }
        }
        ...
        private void init_search_process(string sn, string ct, string labeltype)
        {
            ...
            DataTable initDt = clsUtil.GetServiceData("com.thirautech.mom.pop.popResult.get_palletpack1.dummy", paramsList, clsStatic._serviceSelectURL, ref retvalue);
            ...
            if(initDt.Rows.Count > 0)
            {
                ...
                {
                    if(initDt.Rows[0]["wospec"].ToString() != "")
                    {
                        frmMessage frm = new frmMessage(initDt.Rows[0]["WOSPEC"].ToString().Trim() + "이 필요한 제품입니다.", "AUTOCLOSE");
                        frm.ShowDialog();
                    }
                    ...
                    lblSpec.Text = initDt.Rows[0]["wospec"].ToString();
                }
                else
                {
        ...
        private void pallet_init()
        {
            ...
            DataTable dt = clsUtil.GetServiceData("com.thirautech.mom.pop.popResult.create_palletinit_proc1.dummy", paramsList, clsStatic._serviceUsertURL, ref retvalue);
