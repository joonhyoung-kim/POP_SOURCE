...
//		});
	...
	excelCallBackPost : function(param, data, callbackParam) {
		var that = this.MOMBA004;
		if(param == 'SUCCESS'){
			AUIGrid.clearGridData("grid");
			that.setColDate();
...
//						pivot += "MIN(DECODE (TO_CHAR(A.WORK_DATE,'YYYYMMDD'), '" + planDateArray[i].substring(0, 8) + "',A.WORK_QTY, '')) AS \"" + planDateArray[i].substring(4, 6) + "/" + planDateArray[i].substring(6, 8) + "(" + planDateArray[i].substring(8, 11) + ")" + "\"";
...
//					var key = planDateArray[i].substring(4, 6) + "/" + planDateArray[i].substring(6, 8) + "(" + planDateArray[i].substring(8, 11).toLowerCase() + ")";
