...
//		} 
			...
			that.gridComboBoxSet("popGrid2", tuCommon.contextPath() + "/mom/request/com.thirautech.mom.common.comFacility.dummy"+'?facilityType' + '=' + 'FAC200', "warehouse", "DropDownListRenderer", true);
...
//	},
	...
	comboBox : function(){
		...
		mCommon.comboBoxClickCall("#itemType", function(callBack) {
			$.get(tuCommon.contextPath() + "/mom/request/com.thirautech.mom.common.comCode.dummy",{codeClassId : "ITEM_TYPE"}, // 파라미터
					function(data) {
						options.local = data;
	...
	delCallback : function(param, data, callbackParam){
		...
		if(param == 'SUCCESS'){
			...
			micaCommon.messageBox({type:"danger",  html:Language.lang['MESSAGES10821']});
			console.log(data);
		}
	},
	...
	listCallbackPost : function(param, data){
		...
		if(param == 'SUCCESS'){
			...
			micaCommon.messageBox({type:"success",  html:Language.lang['MESSAGES10692']});
			that.selectDetailList();
		} else {
