...
namespace SmartMOM_POP
{
    public partial class ucMoveProductItemAction : UserControl
    {
        ...
        public ucMoveProductItemAction(string palletid)
        {
            InitializeComponent();
        ...
        private void search_process(string item_id, string qtybuf)
        {
            ...
            for (int i = 0; i < grdMain.Rows.Count; i++)
            {
                ...
                if (item_id == gridbuf)
                {
                    grdMain["ScanQty", i].Value = (int.Parse(grdMain["ScanQty", i].Value.ToString().Replace(",", "")) + int.Parse(qtybuf.Replace(",", ""))).ToString("###,##0");
                    return;
                }
            }
            ...
            grdMain["ScanQty",   nownow].Value = qtybuf.Replace(",", "");
        ...
        private void search_process(string sn, string ct, string palletid, string qtybuf, string itemid)
        {
            ...
            if(checkbuf == "NO")
            {
                ...
                grdMain["ItemId",       nownow].Value = dt.Rows[0]["ITEMID"].ToString();
                grdMain["ItemName",       nownow].Value = dt.Rows[0]["ITEMNAME"].ToString();
                grdMain["ItemTypeName", nownow].Value = dt.Rows[0]["ITEMTYPENAME"].ToString();
                grdMain["CurrentQty",     nownow].Value = dt.Rows[0]["CURRENTQTY"].ToString();
                grdMain["ScanQty",   nownow].Value = qtybuf.Replace(",", "");
                grdMain["UnitCd",       nownow].Value = dt.Rows[0]["UNIT"].ToString();
                grdMain["Specification",       nownow].Value = dt.Rows[0]["SPECIFICATION"].ToString();
                grdMain["FreeOfferName", nownow].Value = dt.Rows[0]["FREEOFFERNAME"].ToString();
                grdMain["Description",       nownow].Value = dt.Rows[0]["DESCRIPTION"].ToString();
            }
        ...
        */
        ...
        private void grdMain_Color_change()
        {
            ...
            for (int i = 0; i < grdMain.Rows.Count; i++)
            {
                currentqty = int.Parse(grdMain["CurrentQty", i].Value.ToString().Replace(",", ""));
                scanqty = int.Parse(grdMain["ScanQty", i].Value.ToString().Replace(",", ""));
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            ...
            if (clsStatic._dialogValue != "")
            {
                ...
                grdMain["ScanQty", grdMain.CurrentRow.Index].Value = (int.Parse(grdMain["ScanQty", grdMain.CurrentRow.Index].Value.ToString().Replace(",", "")) + int.Parse(clsStatic._dialogValue)).ToString("###,##0");
        ...
        private void btnTransfer_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0;i<grdMain.Rows.Count;i++)
            {
                string item_id = grdMain["ItemId", i].Value.ToString().Replace(",", "");
                int qty = int.Parse(grdMain["ScanQty", i].Value.ToString().Replace(",", ""));
