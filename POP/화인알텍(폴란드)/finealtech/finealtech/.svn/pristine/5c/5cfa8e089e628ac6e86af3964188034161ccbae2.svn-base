...
var MOMBA005 = {
	...
	setOptionValue : function() {
		$.ajax({
			...
			type : "GET",
			data : {codeId : "DEMAND_PO_EXCEL_UPLOAD_TYPE"},
			async: false,
			timeout 	: 30000000,
			dataType 	: 'json',
	...
	excelUpload: function() {
		excel_upload(file, 'plan.plan.salesOrderUpload.salesOrderUpload', 'MOMBA005', 'grid', JSON.stringify({uploadType : uploadType}), this.excelProcCallBack);
		micaCommon.splash.show();
	},
	excelCallback : function(param, data, callbackParam){
		var that = this.MOMBA005;
		if(param == "SUCCESS") {
			that.excelUpload();
		} else {
	...
	excelTempDown: function () {
		$.get(tuCommon.contextPath() + '/mom/request/com.thirautech.mom.plan.plan.salesOrderUpload.salesOrderUploadExcelSample.dummy', 
			...
			function(data){
				...
				var option = {fileName:"SALES_ORDER_UPLOAD(ItemPO)_MOMBA005_" + get_current_date("yyyy-mm-dd")}
