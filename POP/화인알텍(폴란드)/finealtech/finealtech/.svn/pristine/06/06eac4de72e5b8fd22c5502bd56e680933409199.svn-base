...
var MOMFB004 = {
	init: function() {
		Language.init(function() {
			mCommon.init("grid", "W201809101424528451005p0V1mvIB77Z", null, null, Language);
		});
		var that = this;
		that.comboBox();
		that.event();
...
//			mCommon.render("grid", "W201809101424528451005p0V1mvIB77Z", mCommon.formGetParam("#form"), function(){});
			setTimeout(function() {
				...
				}, "plan.order.B2BIifrcv.b2biIfRcvCount");
			}, 500);
		});
...
//					fn_b2bi_upload('com.thirautech.mom.plan.order.B2BIifrcv.b2biIfRcv', {vendorSiteCd : vendorSiteCd, b2biType : "PuscsReceivingGERP" }, that.callbackPost);
					...
					mom_ajax("C", "plan.order.B2BIifrcv.b2biIfRcv", JSON.stringify(param), that.callbackPost);
				}
			}});
		});
