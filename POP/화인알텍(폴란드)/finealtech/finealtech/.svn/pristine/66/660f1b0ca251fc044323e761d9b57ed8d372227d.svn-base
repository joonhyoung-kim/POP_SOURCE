...
namespace SmartMOM_POP
{
    public partial class frmPOSearch : Form
    {
        ...
        public frmPOSearch(string wo_state)
        {
            ...
            CultureInfo cultures = new CultureInfo("pl-PL");
            lblToday.Text = DateTime.Now.ToString("yyyy-MM-dd ddd", cultures);
            ...
            if(grdMain.Rows.Count > 0)
            {
                ...
                _wo = grdMain.Rows[0].Cells["WorkOrderId"].Value.ToString();
                _po = grdMain.Rows[0].Cells["ProductOrderId"].Value.ToString();
        ...
        {
            //                                   0.작업지시번호   1.고객사제번      2.품목     3.품명      4.규격           5.지시량      6.잔량수량   7.작업일자         8.양품수량    9.불량수량     10.작업지시유형  11.상태         12.입력기본수량  13.입력방식     14.CT구성수량   15.GT라벨ID      16.CT라벨ID      17.PALLET라벨ID                       
            string[] headerText = new string[] { "WorkOrderId",   "ProductOrderId", "ItemId",  "ItemName", "Spec",          "ConfirmQty", "RemainQty", "PlanDate",        "GoodQty",    "BadQty",      "OrderType",     "OrderState",   "MakeLotQty",    "InputType",    "BoxQty",       "GT LabelID",    "CT LabelID",    "Pallet LabelID"    }; //18
            string[] columnName = new string[] { "WORKORDERID",   "PRODUCTORDERID", "ITEMID",  "ITEMNAME", "SPECIFICATION", "CONFIRMQTY", "REMAINQTY", "PLANDATE",        "QTY",        "BADQTY",      "WOTYPENAME",    "WOSTATENAME",  "POPMAKELOTQTY", "POPINPUTTYPE", "POPCTQTY",     "POPGTLABELID",  "POPCTLABELID",  "POPPALLETLABELID"  };
            string[] columnType = new string[] { "T",             "T",              "T",       "T",        "T",             "T",          "T",         "T",               "T",          "T",           "T",             "T",            "T",             "T",            "T",            "T",             "T",             "T"                 };
                                                                                                                                                                                                                        
            int[] columnWidth    = new int[]   { 190,             125,              150,       310,        230,             80,           80,          130,               80,           80,            100,             230,            100,             100,            100,            100,             100,             100                 };
            ...
            string[] cellAlign = new string[]  { "C",             "C",              "L",       "L",        "L",             "R",          "R",         "C",               "R",          "R",           "C",             "C",            "C",             "C",            "C",            "C",             "C",             "C"                 };
        ...
        private void grid_select()
        {
            ...
            if (rowIndex >= 0)
            {
                _wo = grdMain.Rows[rowIndex].Cells["WorkOrderId"].Value.ToString();
            }
            ...
            if (_wo != "")
            {
                ...
                string popmakeqty = grdMain.Rows[rowIndex].Cells["MakeLotQty"].Value.ToString();
                string popinputtype = grdMain.Rows[rowIndex].Cells["InputType"].Value.ToString();
                string popctqty = grdMain.Rows[rowIndex].Cells["BoxQty"].Value.ToString();

                string popgtlabelid = grdMain.Rows[rowIndex].Cells["GT LabelID"].Value.ToString();
                string popctlabelid = grdMain.Rows[rowIndex].Cells["CT LabelID"].Value.ToString();
                string poppalletlabelid = grdMain.Rows[rowIndex].Cells["Pallet LabelID"].Value.ToString();
                    ...
                    || poppalletlabelid.Trim() == "")
                {
                    ...
                    frmMessage frm1 = new frmMessage("Please check the information of the selected WORKORDER!(Please contact your administrator.)", "AUTOCLOSE");
                    frm1.ShowDialog();
                    return;
                }
        ...
        private void grid_move(int rowindex, string movetype)
        {
            ...
            grdMain.CurrentCell = grdMain.Rows[rowindex].Cells[0];
            _po = grdMain.Rows[rowindex].Cells["ProductOrderId"].Value.ToString();
            _wo = grdMain.Rows[rowindex].Cells["WorkOrderId"].Value.ToString();
        }
        ...
        {
            if(btnTodayFlag.Text == "Today")
            {
                btnTodayFlag.Text = "Past";
        ...
        private void row_change_process(object sender)
        {
