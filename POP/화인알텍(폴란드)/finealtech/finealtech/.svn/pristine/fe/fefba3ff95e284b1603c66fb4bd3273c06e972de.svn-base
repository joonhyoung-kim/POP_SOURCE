			...
			var option = {
					...
					okButton:{
						...
						after:function(){
							...
							mom_ajax("D","common.dataMultiDelTmp", JSON.stringify(param), that.tempInsertCallback, checkedItems, "CANCLE_QTY");
						}
					}
			}
...
//			document.location.href = reportUrl + JSON.stringify(jsonList);
	...
	comboBox: function() {
		...
		//상태
		$.get(tuCommon.contextPath() +"/mom/request/com.thirautech.mom.purchase.purchasing.irregularOrderCancel.comState.dummy", // 호출 URL
			{codeClassId:"MATERIAL_ORDER_STATE"}, // 파라미터
			function(data){
				options.local = data;
	...
	tempInsertCallback : function(param, data, callbackParam, orderType){
		var that = this.MOMCA003;
		var arrayList = [];
		if(param == 'SUCCESS'){
