...
namespace SmartMOM_POP
{
    public partial class ucGanbanSplit : UserControl
    {
        ...
        private void InitMainList()
        {
            ...
            string[] headerText  = new string[] { "라벨ID",   "품목",   "품명",     "출발번호",         "작업지시서",  "PALLET ID", "CT ID", "요청창고", "창고", "이동일",   "이동자", "수량",    "생성일",     "생성자",   "작업자", "사용유무" };
            string[] columnName  = new string[] { "GANBANID", "ITEMID", "ITEMNAME", "DEPARTUREGROUPID", "WORKORDERID", "PALLETID",  "CT",    "FROMSLOC", "SLOC", "MOVEDATE", "MOVEBY", "GOODQTY", "CREATEDATE", "CREATEBY", "NAME",   "USEYN"    };
            string[] columnType  = new string[] { "T",        "T",      "T",        "T",                "T",           "T",         "T",     "T",        "T",    "T",        "T",      "T",       "T",          "T",        "T",      "T"        };

            int[] columnWidth    = new int[]    { 110,        195,      270,        270,                130,           130,         130,     130,        130,    160,        80,       270,       270,          140,        250,      250        };
            bool[] columnVisible = new bool[]   { true,       true,     true,       true,               true,          false,       true,    true,       true,   true,       true,     true,      true,         true,       true,     true       };
            bool[] columnDisable = new bool[]   { true,       true,     true,       true,               true,          true,        true,    true,       true,   true,       true,     true,      true,         true,       true,     true       };
            string[] cellAlign   = new string[] { "C",        "L",      "L",        "L",                "R",           "R",         "R",     "R",        "R",    "C",        "C",      "L",       "L",          "R",        "C",      "C"        };
        ...
        private void ganban_split(bool splitFlag)
        {
            if (lbl_ganbanId.Text == "")
            {
                frmMessage frm1 = new frmMessage("간판 라벨 조회 후 분할 가능합니다.", "AUTOCLOSE");
                frm1.ShowDialog();
                txtSN_Focus();
                return;
            }

            if (lbl_splitQty.Text == "" || lbl_splitQty.Text.Equals("0"))
            {
                frmMessage frm1 = new frmMessage("분할 수량을 입력해 주세요.", "AUTOCLOSE");
                frm1.ShowDialog();
                txtSN_Focus();
                return;
            }
            ...
            {
                frmMessage frm1 = new frmMessage("사용불가 처리된 라벨 입니다.", "AUTOCLOSE");
                frm1.ShowDialog();
                txtSN_Focus();
                return;
            }

            if (int.Parse(lbl_qty.Text.ToString().Replace(",", "")) <= int.Parse(lbl_splitQty.Text.ToString().Replace(",", "")))
            {
                frmMessage frm1 = new frmMessage("분할 수량은 총 수량을 넘거나 같을 수 없습니다.", "AUTOCLOSE");
                frm1.ShowDialog();
                txtSN_Focus();
                return;
            }
            ...
            frmMessage frm = new frmMessage("간판라벨발행을 분할을 실행 하시겠습니까?(분할수량 기준)", "OK_CANCEL");
            DialogResult result = frm.ShowDialog();
            ...
            try
            {
                if (result == DialogResult.OK)
                {
                    ...
                    if (splitFlag)
                    {
                        int pollibleQty = int.Parse(lbl_qty.Text.ToString().Replace(",", ""));
                        int splitqty = int.Parse(lbl_splitQty.Text.ToString().Replace(",", ""));
                        int makeqty = pollibleQty / splitqty;
                        int etc = pollibleQty % splitqty;
        ...
        private void make_split(string labelid, int splitQty, string ganbanId)
        {
            List<Dictionary<string, object>> paramsList = new List<Dictionary<string, object>>();
            Dictionary<string, object> paramsMap = new Dictionary<string, object>();
            paramsMap.Add("p_err_code", "");
