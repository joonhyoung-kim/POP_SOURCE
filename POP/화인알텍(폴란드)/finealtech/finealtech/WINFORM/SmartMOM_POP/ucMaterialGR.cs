...
namespace SmartMOM_POP
{
    public partial class ucMaterialGR : UserControl
    {
        ...
        {
            //                                   1         2           3               4             5              6              7                      8           9   
            string[] headerText = new string[] { "품목",   "품명",     "규격",         "입고량",     "업체",        "박스수량",    "ITEMSTOCKINOUTID",    "LABELYN",  "CHKYN"  , "출발처리번호"    };
            string[] columnName = new string[] { "ITEMID", "ITEMNAME", "ITEMTYPENAME", "QTY",        "VENDOR_NAME", "POPCTQTY",    "ITEMSTOCKINOUTID",    "LABELYN",  "CHKYN"  , "ITEMDEPARTUREID" };
            string[] columnType = new string[] { "T",      "T",        "T",            "T",          "T",           "T",           "T",                   "T",        "T"      , "T"               };
                                                                                                                                                      
            int[] columnWidth    = new int[]   { 230,      250,        180,             150,         180,           100,           90,                    90,         10       , 100               };
            bool[] columnVisible = new bool[]  { true,     true,       true,            true,        true,          true,          false,                 false,      false    , false             };
            bool[] columnDisable = new bool[]  { true,     true,       true,            true,        true,          true,          true,                  true,       true     , true              };
            string[] cellAlign = new string[]  { "L",      "C",        "R",             "R",         "L",           "C",           "C",                   "L",        "L"      , "C"               };
        ...
        private void btnDate_Click(object sender, EventArgs e)
        {
            ...
            {
                frmMessage frm1 = new frmMessage("입고창고가 선택되지 않았습니다.", "AUTOCLOSE");
                frm1.ShowDialog();
                return;
            }
        ...
        private void grdMain_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if(labelYN == "Y")
            {
                frmMessage frm1 = new frmMessage("이미 라벨이 생성된 항목입니다.", "AUTOCLOSE");
                frm1.ShowDialog();
                return;
            }
        ...
        private void btnLabel_Click(object sender, EventArgs e)
        {
            ...
            {
                frmMessage frm1 = new frmMessage("입고된 자재가 없습니다.", "AUTOCLOSE");
                frm1.ShowDialog();
                return;
            }
                ...
                if (result == DialogResult.OK)
                {
                    ...
                    for (int i = 0; i < _gridDt.Rows.Count; i++)
                    {
                        if (_gridDt.Rows[i]["CHKYN"].ToString().Equals("Y") && _gridDt.Rows[i]["LABELYN"].ToString().Equals("N"))
                        {
                            ...
                            {
                                make_ganban(retbuf, _gridDt.Rows[i]["POPCTQTY"].ToString().Replace(",", ""), _gridDt.Rows[i]["ITEMID"].ToString(), _gridDt.Rows[i]["ITEMSTOCKINOUTID"].ToString(), _gridDt.Rows[i]["ITEMDEPARTUREID"].ToString());
                                Thread.Sleep(500);
                            }

                            if (etc > 0)
                            {
                                make_ganban(retbuf, etc.ToString(), _gridDt.Rows[i]["ITEMID"].ToString(), _gridDt.Rows[i]["ITEMSTOCKINOUTID"].ToString(), _gridDt.Rows[i]["ITEMDEPARTUREID"].ToString());
                            }
                        }
                    }
                }
        ...
        private void make_ganban(string labelid, string makeGanban, string itemId, string stockInoutId, string itemDepartureId)
        {
            string ganban = "";
            string depature = "";
            string stockId = "";
