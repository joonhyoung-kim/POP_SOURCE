...
namespace SmartMOM_POP
{
    public partial class ucGanbanUseCancel : UserControl
    {
        ...
        private void InitMainList()
        {
            ...
            bool[] columnDisable = new bool[]   { true,       true,     true,       true,         false,   true       };
        ...
        private void add_Ganban()
        {
            ...
            //간판 정보 조회
            DataTable ganBodyDt = clsUtil.GetServiceData("com.thirautech.mom.pop.popResult.get_ganbanInfoForUseCancelProcess.dummy", paramsList1, clsStatic._serviceSelectURL, ref retvalue);
        ...
        {
            //스캔한 라벨들을 소진취소 처리 하시겠습니까?
            frmMessage frm = new frmMessage("Do you want to cancel the scanned label?", "OK_CANCEL");
            DialogResult result = frm.ShowDialog();
            ...
            if (result == DialogResult.OK)
            {
                for (int i = 0; i < grdMain.Rows.Count; i++)
                {
                    ...
                    string qty = grdMain["Qty", i].Value.ToString();

                    //간판라벨 소진 처리
                    use_cancel_process(ganban, qty);
                }
                //라벨 소진취소 처리가 정상적으로 처리되었습니다.
                frmMessage frm3 = new frmMessage("Label cancellation processing was successful.", "AUTOCLOSE");
                frm3.ShowDialog();
        ...
        private void use_cancel_process(string ganban, string qty)
        {
            ...
            if (resultDt == null)
            {
                //라벨 소진취소 처리가 정상적으로 처리되지 않았습니다.
                frmMessage frm1 = new frmMessage("Label cancellation processing did not process successfully.", "AUTOCLOSE");
                frm1.ShowDialog();
                txtSN_Focus();
                return;
            }
            ...
            {
                //라벨 소진취소 처리가 정상적으로 처리되지 않았습니다.
                frmMessage frm2 = new frmMessage("Label cancellation processing did not process successfully.", "AUTOCLOSE");
                frm2.ShowDialog();
                txtSN_Focus();
                return;
            }
