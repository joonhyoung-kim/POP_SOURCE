...
namespace SmartMOM_POP
{
    public partial class frmLogin : Form
    {
        public frmLogin()
        {
            ...
            InitializeComponent();
            //clsStatic._serviceSelectURL = "http://localhost:8100/TU_Platform/pop/request";
            //clsStatic._serviceUsertURL = "http://localhost:8100/TU_Platform/pop/request/upsert";

            //clsStatic._serviceSelectURL = "http://192.168.0.61:8100/TU_Platform/pop/request";
            //clsStatic._serviceUsertURL = "http://192.168.0.61:8100/TU_Platform/pop/request/upsert";

            //clsStatic._serviceSelectURL = "http://10.10.1.7:8100/TU_Platform/pop/request";
            //clsStatic._serviceUsertURL = "http://10.10.1.7:8100/TU_Platform/pop/request/upsert";

            clsStatic._serviceSelectURL = "http://176.103.174.253:8100/TU_Platform/pop/request";
            clsStatic._serviceUsertURL = "http://176.103.174.253:8100/TU_Platform/pop/request/upsert";
            ...
            if (dt.Rows.Count > 0)
            {
                lblTitle.Text = "SmartPOP Login(" + dt.Rows[0]["USEMANAGEMENT"].ToString() + ")" + " V " + version;
                ...
                clsStatic._INPUTTYE = dt.Rows[0]["INPUTTYPE"].ToString();

				// 김백건 2019.08.08
                // print 관련 정보 전역변수 추가를 위한 설정
                ...
                if (strsLogin.Length == 7)
                {
                    ...
                    if (strsLogin[5] == "")
                    {
                        clsStatic._XPOSITION = "0";
                    } else
                    {
                        clsStatic._XPOSITION = strsLogin[5];
                    }
                    if (strsLogin[6] == "")
                    {
                        clsStatic._YPOSITION = "0";
                    }else
                    {
                        clsStatic._YPOSITION = strsLogin[6];
                    }

                    btnUser.Text = clsStatic._USER_NAME + "(" + clsStatic._USER_ID + ")";

                    if (dt.Rows[0]["USEMANAGEMENT"].ToString() == "GANBAN_PRINT")
                    {
                        btnLine.Text = "Label Publish";
                    }
                    /*
                    else if (strsLogin[2] == "PALLET")
                    {
                    ...
                    */
                    //2019.08.06 조아람 수정
                    else if (clsStatic._USEMANAGEMENT == "STOCK")
                    {
                        btnLine.Text = "WMS";
                    }
                    //=======
        ...
        private void btnLogin_Click(object sender, EventArgs e)
        {
            ...
            {
                clsStatic._badnonDt = clsLabelSet.getBADNONCODEList();
                frmMainProductComplete_Poland frm = new frmMainProductComplete_Poland();
                frm.ShowDialog();
            }
            ...
            else if (clsStatic._USEMANAGEMENT == "GANBAN_PRINT")
            {
                frmLabelPrint frm = new frmLabelPrint();
                frm.ShowDialog();
            }
            else
            {
				...
				//2019.08.06 조아람 수정
                else if (btnLine.Text == "WMS")
                {
                    frmWMSProcess frm = new frmWMSProcess();
                    frm.ShowDialog();
                }
                //========
            }
