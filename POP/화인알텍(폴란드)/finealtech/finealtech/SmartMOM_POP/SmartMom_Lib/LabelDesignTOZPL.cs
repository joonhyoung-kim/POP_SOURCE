...
namespace SmartMom_Lib
{
    public class LabelDesignTOZPL
    {
        ...
        /// </summary>
        /// <param name="x_position">X축 시작점(Pixel)</param> 
        /// <param name="y_position">Y축 시작점(Pixel)</param> ㅉ
        /// <param name="label_value">폰트명</param> 
        /// <returns></returns>
        public string ImageScript(string x_position, string y_position, string label_value)
        {

            string retbuf = "^FO" + x_position.Trim() + "," + y_position.Trim() + "^XGE:" + label_value.Trim() + "^FS";
        ...
        public string Code128Script(string x_position, string y_position, string barcode_module_width, string barcode_height, string landscape, string barcode_text_under_yn, string label_value)
        {
            ...
            else
            {
                ...
                retbuf = "^BY" + barcode_module_width.Trim().Replace(",", ".") + "," + barcode_ratio.Trim().Replace(",", ".") + "," + barcode_height.Trim().Replace(",", ".") + "^FT" + x_position.Trim() + "," + y_position.Trim() + "^BC" + landscape.Trim() + "," + barcode_height.Trim() + "," + barcode_text_under_yn + "^FD" + label_value + "^FS";
                //ean128 test
        ...
        public string Code39Script(string x_position, string y_position, string barcode_module_width, string barcode_ratio, string barcode_height, string landscape, string barcode_text_under_yn, string label_value)
        {
            ...
            {
                retbuf = "^BY" + barcode_module_width.Trim().Replace(",", ".") + "," + barcode_ratio.Trim().Replace(",", ".") + "," + barcode_height.Trim().Replace(",", ".") + "^FT" + x_position.Trim() + "," + y_position.Trim() + "^B3" + landscape.Trim() + ",N," + barcode_height.Trim() + "," + barcode_text_under_yn + ",N^FD" + label_value + "^FS";
            }
        ...
        public string Code93Script(string x_position, string y_position, string barcode_module_width, string barcode_ratio, string barcode_height, string landscape, string barcode_text_under_yn, string label_value)
        {
            ...
            {
                retbuf = "^BY" + barcode_module_width.Trim().Replace(",", ".") + "," + barcode_ratio.Trim().Replace(",", ".") + "," + barcode_height.Trim().Replace(",", ".") + "^FT" + x_position.Trim() + "," + y_position.Trim() + "^BA" + landscape.Trim() + "," + barcode_height.Trim() + "," + barcode_text_under_yn + ",N,N^FD" + label_value + "^FS";
            }
...
^CI28";
            ...
            for (int i = 0; i < _scriptDt.Rows.Count; i++)
            {
                ...
                */
                ...
                if (_scriptDt.Rows[i]["METHODID"].ToString() == "IMAGE")
                {
                    label_script += ImageScript(x_position, y_position, label_value);
                    continue;
                }
            ...
            //Clipboard.SetText(label_script);
...
^CI28";
            ...
            for (int i = 0; i < grdLabelObject.Rows.Count; i++)
            {
                ...
                if (row_check == true)
                {
                    ...
                    if (grdLabelObject["METHODID", i].Value.ToString() == "IMAGE")
                    {
                        label_script += label.ImageScript(x_position, y_position, label_value);
                        continue;
                    }
        ...
        public void bindDictionary_To_ZPL_MAIN(DataTable designDt, Dictionary<string, string> sourceDic, DataSet basicDs, string printPort, object Outport)
        {
            ...
            DataTable basicDt = basicDs.Tables["BASIC_CUR"];
            DataTable combiDt = basicDs.Tables["COMBI_CUR"];
            DataSet macDs = clsLabelSet.basic_info_search(clsStatic._MACADDRESS);
            string strOutport = macDs.Tables["MAC_CUR"].Rows[0][printPort].ToString().Trim();
