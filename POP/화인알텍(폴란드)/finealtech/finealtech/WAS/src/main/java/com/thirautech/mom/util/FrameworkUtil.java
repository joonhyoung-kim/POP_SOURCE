...
public class FrameworkUtil {
	...
	public static Map<String, Object> createParam(Map<String, Object> oldParam) {
		...
		while (iteratorKey.hasNext()) {
			...
			try {
				if(oldParam.get(key) == null) {
					...
					newParam.put(key, URLDecoder.decode(oldParam.get(key).toString(), StandardCharsets.UTF_8.name()));
				}
		...
		if(AuthManager.getSessionAttribute("userId") != null){
			...
			String divisionCd = AuthManager.getSessionAttribute("divisionCd").toString();
			String locale = AuthManager.getSessionAttribute("locale").toString();
			/*String key =  companyCd + divisionCd + locale + "XML";*/
	...
	public static Map<String, Object> createParam(Map<String, Object> oldParam, Map<String, Object> map) {
		Map<String, Object> newParam = new HashMap<String, Object>();
		Iterator<String> iteratorKey = oldParam.keySet().iterator(); 
		while (iteratorKey.hasNext()) {
			String key = iteratorKey.next();
	...
	public static List<Map<String, Object>> createParam(List<Map<String, Object>> oldList) {
		String companyCd = AuthManager.getSessionAttribute("companyCd").toString();
		String divisionCd = AuthManager.getSessionAttribute("divisionCd").toString();
		String locale = AuthManager.getSessionAttribute("locale").toString();
		/*String key =  companyCd + divisionCd + locale + "XML";*/
		...
		for(int i = 0; i < oldList.size(); i++) {
			Map<String, Object> oldMap = oldList.get(i);
			if(i != 0 && (oldMap == null || oldMap.isEmpty())) {
				continue;
			}
			
			Map<String, Object> newMap = FrameworkUtil.createParam(oldMap, map);
...
//			System.out.println("[" + j + "] " + tokens[j] + " :: " + tokens[j].indexOf(":") + " == " + (tokens[j].length() - 1));
			if(tokens[j].indexOf(":") == tokens[j].length() - 1) {
				...
				//System.out.println("#### key = " + tokens[k].substring(0,tokens[k].indexOf(":")).trim() + ", value = " + map.get(tokens[k].substring(0,tokens[k].indexOf(":")).trim()) + "," + tokens[j]);
			} else {
		...
		for (int i = 0; i < pairs.length; i++) {
		    ...
		    try {
		    	map.put(keyValue[0], keyValue[1] == null ? null : keyValue[1].replaceAll("#44#",","));
		    } catch(Exception e) {
		    	map.put(keyValue[0], null);
		    }
		    
		    map.put(keyValue[0], keyValue[1] == null ? null : (keyValue[1].indexOf("") >= 0 ? keyValue[1].replaceAll("#44#",",") : keyValue[1]));
		}
