...
var MOMIB002 = {
					...
					after:function(){
						for(var i = 0; i < checkedItems.length; i++){							
							var param = { parentItemId : checkedItems[i].item.parentItemId,
										  childItemId : checkedItems[i].item.itemId }
							
							if(locationParam.bomType != 'E') {
								mom_ajax("D","reference.itemInfo.bom.bom", JSON.stringify(param), that.listCallbackPost);
							} else {
								mom_ajax("D","reference.itemInfo.ebom.ebom", JSON.stringify(param), that.listCallbackPost);
							}
							
							var param = { 
									parentItemId : checkedItems[i].item.parentItemId,
						...
						if(locationParam.bomType != 'E') {
							mom_ajax("D","reference.itemInfo.bom.bom", JSON.stringify(param), that.listCallbackPost, chkFlag);
						} else {
							mom_ajax("D","reference.itemInfo.ebom.ebom", JSON.stringify(param), that.listCallbackPost, chkFlag);
						}
					}
				}});				
			} else {
	...
	listCallbackPost : function(param, data, callbackParam){
	...
	excelCallbackPost : function(param, data){
		var that = this.MOMIB002;
		if(param == 'SUCCESS'){
			micaCommon.messageBox({type:"success", width:"400", height: "145", html:Language.lang['MESSAGES10692']});
		}else{
