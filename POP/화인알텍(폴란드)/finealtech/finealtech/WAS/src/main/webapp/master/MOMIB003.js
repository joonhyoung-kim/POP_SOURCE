...
var MOMIB003 = {
				...
				micaCommon.messageBox({type:"info", width:"400", height: "145", html:Language.lang['MESSAGES10644'], closeButton:{text:"Close"}, okButton:{text:"OK", 
					after:function(){
						for(var i = 0; i < checkedItems.length; i++){							
							...
							mom_ajax("D","reference.itemInfo.bor.bor", JSON.stringify(param), that.callbackPost, chkFlag);
						}
					}
				}});				
			} else {
		...
		$(document).on("click", "#gridModalPopSaveBtn", function() {
			...
			micaCommon.messageBox({type:"info", width:"400", height: "145", html:Language.lang['MESSAGES11194'], closeButton:{text:"Close"}, okButton:{text:"OK", 
				after:function(){
					...
					mom_ajax(type,"reference.itemInfo.bor.bor", JSON.stringify(param), that.callbackPost, true);
				}
			}});
	...
	comboBox : function() {
		mCommon.comboBoxClickCall("#routeName", function(callBack) {
			$.get(tuCommon.contextPath() + "/mom/request/com.thirautech.mom.reference.itemInfo.bor.route.dummy", function(data){
				micaCommon.comboBox.set("#routeName",{ searchMode:'containsignorecase', autoComplete:true}, {local: data, textName : "text", valueName : "id", readonly: false});
				callBack();
			});
		});
		...
		mCommon.comboBoxClickCall("#resourceCdModal", function(callBack) {
			$.get(tuCommon.contextPath() + "/mom/request/com.thirautech.mom.reference.itemInfo.bor.resource.dummy", function(data){
