				...
				micaCommon.messageBox({type : "info", width : "400", height : "145", html : Language.lang['MESSAGES10655'], closeButton : {text : "Close"}, okButton : {text : "OK", 
					after : function() {
						for(var i = 0; i < checkedItems.length; i++){
							...
							mom_ajax("D","reference.resource.vendor.vendor", JSON.stringify(param), that.callbackPost, chkFlag);
						}
					}
				}});				
			} else {
			...
			micaCommon.messageBox({type : "info", width : "400", height : "145", html : Language.lang['MESSAGES11194'], closeButton : {text : "Close"}, okButton : {text : "OK", 
				after:function(){
					...
					if(type == "C") {
						...
						mom_ajax(type, "reference.resource.vendor.vendor", JSON.stringify(param), that.callbackPost, true);	
					}
...
//		$("#dealEndDateModal").val(data.dealEndDate || "");
	...
	getVendorPop: function() {
		...
		var result = {
				...
				vendorName: $("#vendorNameModal").val(),
				vendorType:$("#vendorTypeModal").val(),
//				dealFlag: $("#dealFlagModal").val(),
				businessNo: $("#businessNoModal").val(),
				businessType :  $("#businessTypeModal").val(),
				businessStatus :  $("#businessStatusModal").val(),
				address : $("#addressModal").val(),
				ceoName: $("#ceoNameModal").val(),
				managerName:$("#managerNameModal").val(),
				telNo:$("#telNoModal").val(),
				faxNo:$("#faxNoModal").val(),
//				dealStartDate: $("#dealStartDateModal").val(),
//				dealEndDate: $("#dealEndDateModal").val(),
				email: $("#emailModal").val(),
				preBuildTerm: $("#preBuildTermModal").val(),
				useYn: $("#useYnModal").val(),
				shipInUseFlag : $("#shipInUseFlagModal").val(),
				departureFlag: $("#departureFlagModal").val(),
				shipInLocation: $("#shipInLocationModal").val(),
	...
	callbackProc : function(param, data, callbackParam) {
		if(param == 'SUCCESS'){
			var that = this.MOMIA0003;
			if(callbackParam == undefined) {
				mom_ajax("C", "reference.resource.vendor.vendorLocationProc", "{}", gvThat.callbackPost, true);
			} else{
				mom_ajax("C", "reference.resource.vendor.vendorLocationProc", JSON.stringify(callbackParam), gvThat.callbackPost, true);
			}
