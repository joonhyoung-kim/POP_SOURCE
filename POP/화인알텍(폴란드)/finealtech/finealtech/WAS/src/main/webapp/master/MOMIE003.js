...
var MOMIE003 = {
	init: function() {		
		...
		var shiftCodeUrl = tuCommon.contextPath() + '/mom/request/com.thirautech.mom.reference.workinghours.shiftCode.shiftCodeClass.dummy';
		...
		$(document).on("click", "#createBtn", function() {
			that.setResourceShiftSchedulePop();
			cudFlag = "L";
			$("#pop").micaModal("show");
				...
				after:function() {
					...
					for(var i=0; i<applyDate.length; i++) {
						var listParam = {
							...
							workPerson : param.workPerson,
							applicationDate : applyDate[i]
						}
		...
		$(document).on("click", "#copyBtn", function() {
			...
			if(checkedItems.length > 1 || checkedItems.length == 0){
				...
				cudFlag = "L";
				that.setResourceShiftSchedulePop(checkedItems[0].item);
			}
			$("#pop").micaModal("show");
			$("#pStartTime, #pEndTime").attr("readonly", "readonly");
		});
				...
				micaCommon.messageBox({type:"info", width:"400", height: "145", html:Language.lang['MESSAGES11743'], closeButton:{text:"Close"}, okButton:{text:"OK", 
					after:function(){
						for(var i = 0; i < checkedItems.length; i++) {
							...
							mom_ajax("D", "reference.workinghours.resourceByShiftPlanSchedule.resourceByShiftPlanSchedule", JSON.stringify(param), that.callback, JSON.stringify(param), callBackChk);
						}
					}
				}});				
			} else {
					...
					after:function(){
						for(var i = 0; i < checkedItems.length; i++) {
							$.ajax({
								...
								type : "GET",
								async: false,
								data : {shiftCodeId : checkedItems[i].item.hShiftCd},
								timeout 	: 30000000,
								dataType 	: 'json',
								contentType : 'application/json; charset=UTF-8',
								success : function(data){
									if(data.length > 0) {
										searchShiftType = data[0].shiftType;
									} else {
							...
							var param = {
								...
								shiftCd : checkedItems[i].item.shiftCd,
								applicationDate : checkedItems[i].item.applyDate,
			...
			micaCommon.messageBox({type:"info", width:"400", height: "145", html:Language.lang['MESSAGES11194'], closeButton:{text:"Close"}, okButton:{text:"OK", 
				after:function() {
					...
					for(var i=0; i<toResourceList.length; i++) {
						var param = that.getCopyResourcePop();
						if(param.fromResource != toResourceList[i].originalItem.value) {
							param.toResource = toResourceList[i].originalItem.value;
							arrayList.push(param);
						}
					}
				...
				after:function() {
					for(var i=0; i<toResource.length; i++) {
						var listParam = {
							...
							rgStartDate : param.rgStartDate,
							rgEndDate : param.rgEndDate,
				...
				after:function() {
					...
					if(param.hResourceGroup != '') {
						$.ajax({
							...
							type : "GET",
							async: false,
							data : {hResourceGroup : param.hResourceGroup},
							timeout 	: 30000000,
							dataType 	: 'json',
							contentType : 'application/json; charset=UTF-8',
							success : function(data){
					...
					param.hResources = "";
					param.hResources = hResources;
					
					// 휴일 적용(설비/설비군) 설비 멀티콤보
					for(var i=0; i<hResourceList.length; i++) {
						if(!hResources.includes("'" + hResourceList[i].originalItem.value + "'")) {
							param.hResources += ",'" + hResourceList[i].originalItem.value + "'";
						} else {
							param.hResources = param.hResources.toString();
						} 
					}
	...
	comboBox : function(){
		...
		// SHIFT코드
		$.get(tuCommon.contextPath() + '/mom/request/com.thirautech.mom.reference.workinghours.shiftCode.shiftCodeClass.dummy', {}, function(data){
			options.local = data;
...
//			applicationDate : $("#pApplicationDate").val(),
			...
			updateBy : $("#pModifier").val(),
			updateDate :$("#pModifyTime").val()
		}
		return result;
	},
	...
	// 설비 복사 팝업에서 저장 버튼 클릭 시
	getCopyResourcePop: function() {
		var result = {
			rStartDate : $("#rStartDate").val(),
			rEndDate : $("#rEndDate").val(),
			fromResource : $("#fromResource").val(),
			toResource : $("#toResource").val()
		}
		return result;
	},
	...
	// 휴일 적용 팝업에서 저장 버튼 클릭 시
	getHolidayPop: function() {
		var result = {
			hStartDate : $("#hStartDate").val(),
			hEndDate : $("#hEndDate").val(),
			hResource : $("#hResource").val(),
