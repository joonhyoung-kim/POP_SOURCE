...
namespace SmartMOM_POP
{
    public partial class ucMoveProductItemActionNew : UserControl
    {
        ...
        private void scan_search(string sn, string ct, string palletid, string labeltype)
        {
            ...
            for (int i = 0; i < grdMain.Rows.Count; i++)
            {
                ...
                if (_sn == gridbuf)
                {
                    ...
                    frmMessage2 frm1 = new frmMessage2("이미 스캔한 라벨입니다.", "AUTOCLOSE");
                    frm1.ShowDialog();
                    txtSN_Focus();
                    return;
                }
            ...
            if(sn == "" && ct != "" && palletid == "")
            {
                ...
                {
                    frmMessage2 frm = new frmMessage2("입력한 박스포장라벨(" + ct + ")는 간판라벨이 발행되어 있어 공정재고 이동이 불가능합니다.", "AUTOCLOSE");
                    frm.ShowDialog();
                    return;
                }
            }
            ...
            */
            ...
            if (dt == null)
            {
                frmMessage2 frm = new frmMessage2("현재창고(" + clsStatic._FROMLINE_DESC + ")에 해당 라벨이 존재하지 않거나 비사용 처리된 라벨입니다. 라벨정보를 확인하여 주세요!", "AUTOCLOSE");
                frm.ShowDialog();
                txtSN_Focus();
                return;
            }
        ...
        private void move_transfer(string move_id)
        {
            ...
            if (moveDt == null)
            {
                frmMessage2 frm1 = new frmMessage2("이동처리가 정상적으로 처리되지 않았습니다.", "AUTOCLOSE");
                frm1.ShowDialog();
                txtSN_Focus();
                return;
            }
            ...
            {
                frmMessage2 frm2 = new frmMessage2("이동처리가 정상적으로 처리되지 않았습니다.", "AUTOCLOSE");
                frm2.ShowDialog();
                txtSN_Focus();
                return;
            }
            ...
            frmMessage2 frm3 = new frmMessage2("이동처리가 정상적으로 처리되었습니다.", "AUTOCLOSE");
            frm3.ShowDialog();
            init_process();
            txtSN_Focus();
            return;
        ...
        private void btnTransfer_Click(object sender, EventArgs e)
        {
            ...
            frmMessage2 frm = new frmMessage2("입력한 품목들을 이동처리 하시겠습니까?", "OK_CANCEL");
            DialogResult result = frm.ShowDialog();
            ...
            if (clsStatic._STOCK_TYPE == "MRTN")
            {
                for (int i = 0; i < grdMain.Rows.Count; i++)
                {
                    ...
                    if (qty <= 0)
                    {
                        frmMessage2 frm3 = new frmMessage2("반납수량은 0 보다 커야 합니다.", "AUTOCLOSE");
                        frm3.ShowDialog();
                        return;
                    }
            ...
            if(retbuf == "NG")
            {
                frmMessage2 frm1 = new frmMessage2("현재 이동처리가 진행중입니다. 잠시후 이동처리 진행 하여 주세요.", "AUTOCLOSE");
                frm1.ShowDialog();
                txtSN_Focus();
                return;
            }
            ...
            {
                deleteTmp(_move_id);
                frmMessage2 frm1 = new frmMessage2("이동 처리시 오류가 발생하였습니다. 다시 실행하여주십시오.", "AUTOCLOSE");
                frm1.ShowDialog();
            }
