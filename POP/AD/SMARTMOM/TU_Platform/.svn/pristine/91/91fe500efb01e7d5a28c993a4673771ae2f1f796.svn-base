...
var MOMXX000 = {
	...
	event: function() {
		...
		$(document).on('click', '#excelDownBtn', function() {
			...
			for(var i = 0; i < items.length; i++) {
				...
				items[i]['popUpRead'] = items[i]['popUpRead'] == 1 ? '수정' : (items[i]['popUpRead'] == 2 ? '등록/수정' : '');
			}
		...
		$(document).on('click', '#saveBtnEX1', function() {	
		    ...
		    reader.onload = function() {
		        ...
		        excel_upload_new(grid1, data, function() {
		        	console.log('success');
		        	$('#excelPop1').momModal('hide');
		        });
		    };
	...
	createParam: function(param/*, items, columnFixed*/) {
		...
		/*gridProperty = JSON.parse(gridProperty);
		if(
			   pageName == 'PSI' 
			|| pageName == '설비가동율'
		) {
			gridProperty['enableCellMerge'] = true;
			gridProperty['cellMergePolicy'] = 'withNull';
			gridProperty['rowSelectionWithMerge'] = true;
		}
		if(
			   pageName == '자재이동현황_공정이동현황' 
			|| pageName == '매출마감현황'
			|| pageName == '매입월마감현황'
		) {
			gridProperty['showFooter'] = true;
			gridProperty['footerHeight'] = 24;
		}*/
		
		if($('#cellMergeId').val()) {
			gridProperty += (
				',"enableCellMerge":true,"cellMergePolicy":"withNull","rowSelectionWithMerge":true'
			);
		}
		if($('#showFooterId').val()) {
			gridProperty += (
				',"showFooter":true,"footerHeight":24'
			);
		}
		gridProperty += '}';
	...
	createParamExcel: function(param/*, items, columnFixed*/) {
		...
		var gridProperty = '{'
			...
			+ '#44##22#showRowNumColumn#22#:#22#' + $('#noId').val() + '#22'
			...
			+ '#44##22#displayTreeOpen#22#:true'
			+ '#44##22#enableCellMerge#22#:true'
			+ '#44##22#cellMergePolicy#22#:#22#withNull#22#'
			+ '#44##22#rowSelectionWithMerge#22#:true'
			+ '}';
	...
	retrieve: function() {
		...
		$.get(momWidget.contextPath() + queryId, param, function(data) {
			...
			if(data.length > 0) {
				...
				/*if(data[0]['columnIndex'] == undefined) {
					data[0]['columnIndex'] = '0';
				}*/
				/*for(var i = 0; i < data.length; i++) {
					...
					if(data[i]['message'] != undefined && (data[i]['message'] == 'FILTER' || data[i]['message'] == 'ACTION')) {
						//data[i]['columnIndex'] = '';
					}
				}*/
			}
	...
	preView: function() {
		...
		$.get(momWidget.contextPath() + queryId, param, function(data) {
			...
			columnLayout1 	= JSON.parse(data[0].columnProperty);
			...
			$.get(momWidget.contextPath() + queryId, param, function(data2) {
				...
				for(var idx = 0; idx < columnLayout1.length; idx++) {
					...
					// Begin Color
					if(columnLayout1[i]['color'] != undefined && $.trim(columnLayout1[i]['color']) != '') {
						const idx = i;
	...
	widgetUpload: function(fileId, url, page, grid, param1, callBack) {
		...
		for(var i = 0; i < columnLayout.length; i++) {
	    	if(typeof columnLayout[i].excelTemplateHide == 'undefined' || columnLayout[i].excelTemplateHide) {
	    		if(columnLayout[i].dataField == 'Edit') {
	    			continue;
	    		}
	    		
		    	var key = index.toString();
		    	var value = columnLayout[i].dataField;
		    	param2[key] = value;
