...
//						return retStr;
//					},
//					columnTempleteData : "ERRORMATERIAL"
//					
//				},
//				{
...
//						valueField : "name"
//					},
//					columnTempleteData : "STOCK INIT"
//				}
//			);
		
//			AUIGrid.changeColumnLayout("grid", gridColumn);
			
			AUIGrid.setColumnPropByDataField( "grid", "reasonCd", {
				style:"columnStyle",
				editRenderer : {
					type : "DropDownListRenderer",
					autoCompleteMode : true,
			...
			AUIGrid.setColumnPropByDataField( "grid", "updateQty", {
				style:"columnStyle",
			});
		...
		$(document).on("click", "#excelDownBtn", function() {
			mCommon.auiGridExcelExport("grid", {fileName: "OTHERS_IN_OUT_FINISHED_MOMCE002_" + get_current_date("yyyy-mm-dd")});
		});
		// 보정 버튼
		$(document).on("click", "#correctBtn", function(){
			...
			for(var i = 0; i < checkedItems.length; i++) {
				...
				if(checkedItems[i].item.stateTime == "" || checkedItems[i].item.stateTime == null) {
					micaCommon.messageBox({type:"warning", width:"400", height: "145", html: Language.lang['MESSAGES10487']});
					return;
				}
				
				if(checkedItems[i].item.updateQty == "" || checkedItems[i].item.updateQty == null) {
