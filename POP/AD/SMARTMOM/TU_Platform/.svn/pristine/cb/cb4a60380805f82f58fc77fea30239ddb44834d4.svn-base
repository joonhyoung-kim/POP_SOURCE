...
//			that.chart();
		...
		if(indexInfo != undefined && indexInfo['op'] == 'saveAllBtn4') {
			momWidget.findBtnClicked(3, false, {groupType: that.groupType}, function(result, data) {
				if(result != 'SUCCESS') {
					return;
				}
		...
		if(indexInfo != undefined && indexInfo['op'] == 'saveBtnEP5') {
			momWidget.findBtnClicked(4, false, {groupType: that.groupType, dataType: that.dataType}, function(result, data) {
				if(result != 'SUCCESS') {
					return;
				}
...
//		momWidget.findBtnClicked(1, false, this.initParam, undefined, undefined, this);
		...
		$(document).on('click', '#actionBtn2', function() {
			momWidget.findBtnClicked(3, false, undefined, function(result, data) {
				if(result != 'SUCCESS') {
					return;
				}
