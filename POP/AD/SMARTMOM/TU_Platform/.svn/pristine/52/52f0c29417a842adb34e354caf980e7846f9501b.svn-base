var MOMCA003_1 = {
	...
	init: function() {         
		...
		if(indexInfo != undefined && indexInfo['op'] == 'changeRequestTimeBtn1' && indexInfo['sequence'] == 1) {
			var checkedItems = AUIGrid.getCheckedRowItems(momWidget.grid[0]);
			for(var i=0; i<checkedItems.length; i++) {
				if(checkedItems[i].item.orderState == "CANCEL") {
					this.initMessage = Language.lang['MESSAGES11462'];
					return;
				}
			}
		}
		
		if(indexInfo != undefined && indexInfo['op'] == 'orderCancelBtn1' && indexInfo['sequence'] == 1) {
			var checkedItems = AUIGrid.getCheckedRowItems(momWidget.grid[0]);
			for(var i=0; i<checkedItems.length; i++) {
				if(checkedItems[i].item.orderState == "CANCEL") {
					this.initMessage = Language.lang['MESSAGES10451'];
					return;
				}
			}
		}
