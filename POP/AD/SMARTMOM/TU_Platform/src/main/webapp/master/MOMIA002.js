...
var MOMIA002 = {
	...
	comboBox : function() {
		...
		// 설비위치
		$.get(tuCommon.contextPath() + "/mom/request/com.thirautech.mom.common.comCode.dummy", // 호출 URL
				{ codeClassId : "EQM_LOCATION"}, // 파라미터
				function(data) {
					options.local = data;
					micaCommon.comboBox.set("#placeCdModal",comboOptions, options);
				}
		);
	},
	...
	cudResourceGroup : function (type, data) {
		...
		mom_ajax(type, "reference.resource.facility.facility_group", JSON.stringify(param), that.callbackPost, gridType);
	},
	cudResource : function (type, data) {
		...
		mom_ajax(type, "reference.resource.facility.facility", JSON.stringify(param), that.callbackPost, gridType);
	},
