...
var MOMJA094 = {
	...
	event: function() {
		...
		$(document).on("click", "#pCreateBtn", function() {
			...
			mom_ajax('R', 'admin.itemStockControl.chkItemStockControl', {deptCd : $("#pDeptName").val()}, function(result, data) {
				// 등록 일 때만 Validation 체크(수정은 통과)
				if(type == "C" && data.length > 0) {
					micaCommon.messageBox({type : "warning", width : "400", height : "145", html : Language.lang['MESSAGES12450']});
					return;
				} 
				
				// 수정이거나 등록 시에 이미 등록된 데이터가 아닐경우엔 실행
				if(type == "U" || data.length == 0) {
