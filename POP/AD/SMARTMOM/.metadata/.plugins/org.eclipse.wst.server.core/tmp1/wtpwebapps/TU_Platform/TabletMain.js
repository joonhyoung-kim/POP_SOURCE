...
var TabletMain = {
	init: function() {
		...
		if(newPassword != $("#newPasswordConfirm").val()) {
			mCommon.messageBox({type: "fail", title: "password", html : "password not matched."});
			return;
		} 
		...
		$.ajax({
			...
			success: function(data) {
				if(data.result == "success") {
					mCommon.messageBox({type: "fail", title: "password", html : "password changed."});	
					$("#passwordPop").micaModal("hide");
				} else {
					mCommon.messageBox({type: "fail", title: "password", html : "check current password."});
				}
			}
		});
	},
